...
failure message. It is also the reason every type is declared inline at the top
of `app.ts` rather than in a `types.ts` beside it, which is the first tidying
