...
`build.mjs` inlines four things into `src/index.html`. Three come from `src/`.
The fourth, `dist/app.js`, is `src/app.ts` compiled and minified by esbuild, and
it is the only input the build does not read from `src/`.
