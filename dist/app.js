"use strict";const NUTS=DATA.nutrients,FOODS=DATA.foods,GROUPS=[{id:"macro",label:"Macronutrients",icon:I.macro},{id:"fats",label:"Omega & fats",icon:I.fats},{id:"amino",label:"Amino acids",icon:I.amino},{id:"vitamin",label:"Vitamins",icon:I.vit},{id:"mineral",label:"Minerals",icon:I.min},{id:"carbdetail",label:"Carbohydrate detail",icon:I.carb},{id:"acids",label:"Organic acids",icon:I.acid},{id:"plant",label:"Plant compounds",icon:I.plant},{id:"other",label:"Anti-nutrients",icon:I.other}],VNUTS=NUTS.filter(e=>!e.evidence),IDX=new Map(VNUTS.map((e,t)=>[e.id,t])),CATS=[...new Set(FOODS.map(e=>e.cat))].sort(),slugify=e=>`${e.name} ${e.state||""}`.toLowerCase().trim().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,""),SLUGS=FOODS.map(slugify),BY_SLUG=new Map(SLUGS.map((e,t)=>[e,t])),BY_ID=new Map(NUTS.map(e=>[e.id,e])),nutOpt=e=>BY_ID.get(e),nut=e=>{const t=nutOpt(e);if(!t)throw new Error(`unknown nutrient id: ${e}`);return t},foodAt=e=>{const t=FOODS[e];if(!t)throw new Error(`no food at index ${e}`);return t},slugAt=e=>{const t=SLUGS[e];if(t===void 0)throw new Error(`no food at index ${e}`);return t},foodBySlug=e=>{const t=BY_SLUG.get(e);return t===void 0?void 0:FOODS[t]},groupOf=e=>{const t=GROUPS.find(n=>n.id===e);if(!t)throw new Error(`unknown nutrient group: ${e}`);return t},isGroup=e=>GROUPS.some(t=>t.id===e),RENAMED={"navy-beans-cooked":"haricot-beans-cooked"},currentSlug=e=>RENAMED[e]||e,NOTES=DATA.notes||[],NOTE_AT=new Map;for(const e of NOTES)for(const[t,n]of Object.entries(e.cells||{}))for(const a of n)NOTE_AT.set(`${t} ${a}`,e);const noteFor=(e,t)=>NOTE_AT.get(`${slugAt(e)} ${t}`)||null,ABSORB_NOTES=new Set(["oxalate-high","oxalate-low"]),noteMark=e=>`<sup class="fnote" aria-hidden="true">${esc(e.marker)}</sup><span class="sr">, ${esc(e.short)}</span>`,INTERACTIONS=X.interactions,AFFECTING=new Map;for(const e of INTERACTIONS)for(const t of e.affects)AFFECTING.set(t,[...AFFECTING.get(t)||[],e]);const PREPARING=new Map;for(const e of PREP.preparations)for(const t of e.applies)PREPARING.set(t,[...PREPARING.get(t)||[],e]);const preparing=e=>PREPARING.get(e)||[],prepMeasured=(e,t)=>e.measuredIn.includes(t),ACTING=new Map;for(const e of INTERACTIONS)e.agent.kind==="nutrient"&&ACTING.set(e.agent.id,[...ACTING.get(e.agent.id)||[],e]);const affecting=e=>AFFECTING.get(e)||[],acting=e=>ACTING.get(e)||[],hasInteractions=e=>affecting(e).length>0||acting(e).length>0,affectedLabels=e=>e.affects.map(t=>nutOpt(t)?.label??t),RANK_DEPTH=10;function sourceOf(e){const t=[];for(const n of VNUTS){if(!hasInteractions(n.id))continue;const a=val(e,n.id);if(!(a===null||a<=0))if(n.dv)a/n.dv>=.1&&t.push(n.id);else{const s=IDX.get(n.id);if(s===void 0)continue;let o=0;for(const i of FOODS){const d=i.v[s];if(typeof d=="number"&&d>a&&++o>=RANK_DEPTH)break}o<RANK_DEPTH&&t.push(n.id)}}return t}function gapEvidence(e){const t=nutOpt(e);if(!t||t.evidence)return null;let n=0,a=0,s=0,o=0,i=null,d=null;return FOODS.forEach((r,p)=>{const c=val(r,e);if(c===null){s++;return}if(c===0){a++;return}if(n++,noteFor(p,e)?.id==="fortified"){o++;return}if(t.dv){const u=c/t.dv*100;(i===null||u>i)&&(i=u,d=r.name)}}),{above:n,zero:a,unassayed:s,of:FOODS.length,fortified:o,bestPc:i,bestFood:d}}function gapEvidenceText(e){const t=e.nutrients.map(n=>{const a=gapEvidence(n);if(!a)return"";const s=nutOpt(n)?.label??n,o=[`${a.above} of ${a.of} foods ${a.above===1?"has":"have"} any at all`];return a.fortified&&o.push(`${a.fortified} of those ${a.fortified===1?"is":"are"} fortification rather than the food`),a.zero&&o.push(`${a.zero} were measured and found to contain none`),a.unassayed&&o.push(`${a.unassayed} ${a.unassayed===1?"was":"were"} never assayed`),a.bestPc!==null&&a.bestFood&&o.push(`the best unfortified is ${esc(a.bestFood)}, at ${Math.round(a.bestPc)}% of a daily value per 100 g`),`<li><b>${esc(s)}:</b> ${o.join("; ")}.</li>`}).filter(Boolean);return t.length?`<ul class="gapev"><li class="gapevhead">Measured from this table,
    not quoted:</li>${t.join("")}</ul>`:""}function agentLabel(e){return e.kind==="nutrient"?nutOpt(e.id)?.label??e.id:e.kind==="food"?foodBySlug(e.slug)?.name??e.slug:e.label}const BUILTIN_LENSES=[{id:"essentials",name:"⭐ Essentials",ids:["kcal","protein","carbs","fiber","fat","satfat","ala","la","vitc","vite","vitk","ca","fe","mg","k","zn","na"],why:"The core nutrients most people are likely to care about, keeping the table concise."},{id:"amino",name:"Protein & Amino Acids",ids:["protein","his","ile","leu","lys","met","cys","phe","tyr","thr","trp","val"],why:"Total protein alongside the essential amino acids and their precursors."},{id:"fats",name:"Fats & Omegas",ids:["fat","satfat","mufa","pufa","ala","la","palmitoleic","oleic","epa","dha"],why:"A detailed breakdown of dietary fats, including saturated, monounsaturated, polyunsaturated, and all major omegas."},{id:"carbs",name:"Fibre & Carbohydrates",ids:["carbs","fiber","solfibre","insolfibre","resstarch","starch","sugars","fructose","glucose","sucrose","inulin","raffinose","stachyose"],why:"Total carbohydrates and fibre, broken down into specific sugars, starches, and complex oligosaccharides."},{id:"iron",name:"Iron & Absorption",ids:["fe","vitc"],why:"Plant iron is non-haem and poorly absorbed on its own. Vitamin C in the same meal can multiply uptake severalfold."},{id:"bone",name:"Bone Health",ids:["ca","vitd","vitk","mg","p"],why:"Calcium is only half the story: vitamin D governs how much you absorb, vitamin K directs it into bone, and magnesium and phosphorus build the mineral itself."},{id:"methyl",name:"B12 & Folate",ids:["b12","b9","b6","chol"],why:"The nutrients that keep homocysteine in check. B12 is the critical gap on a vegan diet."},{id:"antiox",name:"Antioxidants",ids:["vitc","vite","se","betacar","luteinzea","lycopene","anthocyanidins","flavan3ols","flavonols","coq10","coq9","melatonin","squalene","ergothioneine"],why:"Nutrients and bioactive compounds that limit oxidative damage."},{id:"phyto",name:"Plant Compounds",ids:["phytosterols","anthocyanidins","flavan3ols","flavonols","lignans","proanthocyanidins","glucosinolates","coq10","coq9","melatonin","squalene","ergothioneine"],why:"A comprehensive look at health-promoting phytochemicals and bioactives."},{id:"electro",name:"Electrolytes",ids:["na","k","ca","mg"],why:"The minerals governing fluid balance, nerve signalling and muscle contraction."},{id:"minabs",name:"Mineral Absorption",ids:["phytate","oxalate","oxalate_sol","fe","ca","zn"],why:"Anti-nutrients that can bind to essential minerals and inhibit their absorption."},{id:"all",name:"All nutrients",ids:["__ALL__"],why:"Shows the full available nutrient set in the entire database."}],DAY_MAX_G=5e3,DEFAULT_G=100,DEFAULT_KG=70,S={groups:new Set(GROUPS.map(e=>e.id)),sort:{id:"__name",dir:1},q:"",cat:"",sel:0,favs:new Set,favsOnly:!1,dv:!1,basis:"g",view:"table",tab:"overview",chartNut:"protein",dark:!1,lens:"essentials",custom:[],day:[],kg:DEFAULT_KG,wUnit:"kg",dayTab:"inputs"},clampG=e=>{const t=Math.round(Number(e));return isFinite(t)&&t>0?Math.min(t,DAY_MAX_G):0},clampKg=e=>{const t=Number(e);return!isFinite(t)||t<=0?DEFAULT_KG:Math.round(Math.min(Math.max(t,30),250)*10)/10},LB_PER_KG=2.2046226218,LB_PER_ST=14,kgToStLb=e=>{const t=Math.round(e*LB_PER_KG);return{st:Math.floor(t/LB_PER_ST),lb:t%LB_PER_ST}},stLbToKg=(e,t)=>(e*LB_PER_ST+t)/LB_PER_KG;function weightLabel(){if(S.wUnit!=="stlb")return`${+S.kg.toFixed(1)} kg`;const{st:e,lb:t}=kgToStLb(S.kg);return`${e} st ${t} lb`}const LS_KEY="vegan-nutrients:v1";let storageOK=!0;function savePrefs(){if(storageOK)try{localStorage.setItem(LS_KEY,JSON.stringify({favs:[...S.favs],groups:[...S.groups],sort:S.sort,dv:S.dv,basis:S.basis,dark:S.dark,lens:S.lens,custom:S.custom,favsOnly:S.favsOnly,cat:S.cat,chartNut:S.chartNut,day:S.day,kg:S.kg,wUnit:S.wUnit}))}catch{storageOK=!1}}const lensById=e=>e&&[...BUILTIN_LENSES,...S.custom].find(t=>t.id===e)||null,lensIds=()=>new Set(lensById(S.lens)?.ids||[]);function loadPrefs(){let e;try{e=JSON.parse(localStorage.getItem(LS_KEY)||"null")}catch{storageOK=!1;return}if(!(!e||typeof e!="object")){if(Array.isArray(e.favs)&&(S.favs=new Set(e.favs.map(currentSlug).filter(t=>typeof t=="string"&&BY_SLUG.has(t)))),Array.isArray(e.groups)){const t=e.groups.filter(isGroup);t.length&&(S.groups=new Set(t))}e.sort&&(e.sort.id==="__name"||BY_ID.has(e.sort.id))&&(S.sort={id:e.sort.id,dir:e.sort.dir===1?1:-1}),typeof e.dv=="boolean"&&(S.dv=e.dv),(e.basis==="g"||e.basis==="kcal")&&(S.basis=e.basis),typeof e.dark=="boolean"&&(S.dark=e.dark),CATS.includes(e.cat)&&(S.cat=e.cat),IDX.has(e.chartNut)&&(S.chartNut=e.chartNut),Array.isArray(e.day)&&(S.day=e.day.filter(t=>t&&typeof t.slug=="string").map(t=>({slug:currentSlug(t.slug),g:clampG(t.g)})).filter(t=>BY_SLUG.has(t.slug))),typeof e.kg=="number"&&isFinite(e.kg)&&(S.kg=clampKg(e.kg)),(e.wUnit==="kg"||e.wUnit==="stlb")&&(S.wUnit=e.wUnit),Array.isArray(e.custom)&&(S.custom=e.custom.filter(t=>t&&typeof t.name=="string"&&Array.isArray(t.ids)).map(t=>({id:String(t.id||""),name:t.name.slice(0,40),ids:t.ids.filter(n=>typeof n=="string"&&BY_ID.has(n)),...typeof t.why=="string"&&t.why?{why:t.why.slice(0,240)}:{}})).filter(t=>t.id&&t.ids.length)),typeof e.lens=="string"&&lensById(e.lens)&&(S.lens=e.lens),S.favsOnly=e.favsOnly===!0&&S.favs.size>0}}const $=e=>{const t=document.querySelector(e);if(!t)throw new Error(`missing element: ${e}`);return t},$opt=e=>document.querySelector(e),targetEl=e=>e.target instanceof HTMLElement?e.target:null,targetInput=e=>e.target instanceof HTMLInputElement?e.target:null,targetAnyEl=e=>e.target instanceof Element?e.target:null,ESCAPES={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"},esc=e=>String(e).replace(/[&<>"']/g,t=>ESCAPES[t]??t),say=e=>{$("#live").textContent=e},GROUP_AT=new Map(GROUPS.map((e,t)=>[e.id,t])),COL_ORDER=(()=>{const e=NUTS.map((t,n)=>({...t,i:n})).sort((t,n)=>(GROUP_AT.get(t.group)??0)-(GROUP_AT.get(n.group)??0));for(const t of e.filter(n=>n.after))e.findIndex(a=>a.id===t.after)!==-1&&(e.splice(e.indexOf(t),1),e.splice(e.findIndex(a=>a.id===t.after)+1,0,t));return e})(),cols=()=>{if(S.lens==="all")return COL_ORDER;if(S.lens){const e=lensById(S.lens);if(e)return COL_ORDER.filter(t=>e.ids.includes(t.id))}return COL_ORDER.filter(e=>S.groups.has(e.group))},val=(e,t)=>{const n=IDX.get(t);if(n===void 0)throw new Error(`unknown nutrient id: ${t}`);return e.v[n]??null},GRAMS_PER=100,KCAL_BASIS=100;function gramsPer100kcal(e){const t=val(e,"kcal");return t?KCAL_BASIS/t*GRAMS_PER:null}const basisLabel=()=>S.basis==="kcal"?"per 100 kcal":"per 100 g";function shown(e,t){if(t.evidence)return null;const n=val(e,t.id);if(S.basis==="g"||t.id==="kcal"||n===null)return n;const a=val(e,"kcal");return a?n/a*KCAL_BASIS:null}const ev=(e,t)=>EV[e]?.cells[t],evFood=e=>EV[e],GRADE_ORDER=["exact","close","proxy"],evGrade=(e,t)=>{const n=evFood(e)?.matches;if(!t||!n)return;let a;for(const s of t.sources||[]){const o=n[s];o&&(!a||GRADE_ORDER.indexOf(o)>GRADE_ORDER.indexOf(a))&&(a=o)}return a},evText=(e,t)=>{if(!e)return"no data";switch(e.state){case"measured":return e.value.toFixed(t);case"range":return`${e.low.toFixed(t)} to ${e.high.toFixed(t)}`;case"estimated":return e.value.toFixed(t);case"trace":return"trace";case"not-detected":return"none detected";case"not-measured":return"not measured"}},evSortKey=(e,t)=>{const n=ev(e,t);return n?n.state==="measured"||n.state==="estimated"?n.value??null:n.state==="range"?(n.low+n.high)/2:null:null},NAME_COUNT=FOODS.reduce((e,t)=>e.set(t.name,(e.get(t.name)||0)+1),new Map),fullName=e=>(NAME_COUNT.get(e.name)??0)>1&&e.state?`${e.name}, ${e.state}`:e.name,isFav=e=>S.favs.has(slugAt(e));function toggleFav(e){isFav(e)?S.favs.delete(slugAt(e)):S.favs.add(slugAt(e)),S.favsOnly&&!S.favs.size&&(S.favsOnly=!1),savePrefs()}function fmtText(e,t){return e===null?"n/a":S.dv&&t.dv?e===0?"0%":Math.round(e/t.dv*100)+"%":e===0?"0":e.toFixed(t.dp)}function fmt(e,t){return e===null?'<span class="na">n/a</span>':fmtText(e,t)}const FAO_PATTERN=[{label:"Histidine",ids:["his"],mg:15},{label:"Isoleucine",ids:["ile"],mg:30},{label:"Leucine",ids:["leu"],mg:59},{label:"Lysine",ids:["lys"],mg:45},{label:"Methionine + cysteine",ids:["met","cys"],mg:22},{label:"Phenylalanine + tyrosine",ids:["phe","tyr"],mg:38},{label:"Threonine",ids:["thr"],mg:23},{label:"Tryptophan",ids:["trp"],mg:6},{label:"Valine",ids:["val"],mg:39}];function proteinQuality(e){const t=val(e,"protein");if(!t||t<1)return null;const n=[];for(const o of FAO_PATTERN){let i=0;for(const r of o.ids){const p=val(e,r);if(p===null)return null;i+=p}const d=i*1e3/t;n.push({label:o.label,pc:d/o.mg*100})}if(n.some(o=>!isFinite(o.pc)))return null;const a=n.reduce((o,i)=>i.pc<o.pc?i:o),s=val(e,"kcal");return{score:Math.round(a.pc),limiting:a.label,perKcal:s!==null&&s>0?t/s*100:null}}function omegaRatio(e){const t=val(e,"la"),n=val(e,"ala");return!t||!n?null:t>=n?{a:t/n,flip:!1}:{a:n/t,flip:!0}}const dayEntries=()=>S.day.flatMap(e=>{const t=BY_SLUG.get(e.slug);if(t===void 0)return[];const n=FOODS[t];return n?[{...e,i:t,f:n}]:[]}),dayContributors=()=>dayEntries().filter(e=>e.g>0),dayGrams=()=>dayContributors().reduce((e,t)=>e+t.g,0);function dayTotals(){const e=dayContributors();return VNUTS.map(t=>{let n=0,a=0;const s=new Set;for(const o of e){const i=val(o.f,t.id);if(i===null)continue;a++,n+=i*o.g/100;const d=noteFor(o.i,t.id);d&&s.add(d)}return{n:t,total:a?n:null,from:a,of:e.length,partial:a>0&&a<e.length,notes:[...s]}})}const totalOf=(e,t)=>{const n=IDX.get(t),a=n===void 0?void 0:e[n];if(!a)throw new Error(`no day total for nutrient id: ${t}`);return a},PROTEIN_G_PER_KG=.66,aaTargets=e=>FAO_PATTERN.map(t=>({label:t.label,ids:t.ids,target:t.mg*PROTEIN_G_PER_KG*e/1e3}));function dayAminoAcids(e){const t=dayContributors();return t.length?aaTargets(S.kg).map(n=>{const s=t.every(o=>n.ids.every(i=>val(o.f,i)!==null))?n.ids.reduce((o,i)=>{const d=totalOf(e,i).total;return o===null||d===null?null:o+d},0):null;return s===null?{...n,got:null,pc:null}:{...n,got:s,pc:s/n.target*100}}):[]}function dayProteinQuality(e){const t=dayContributors();if(!t.length)return null;const n=FAO_PATTERN.flatMap(s=>s.ids);return t.every(s=>n.every(o=>val(s.f,o)!==null))?proteinQuality({v:e.map(s=>s.total)}):null}const A_BUDGET=new Set(["kcal","carbs","fat","satfat","na"]);function dayStanding(e){const t=e.filter(a=>a.n.dv!==null&&a.n.dv>0&&a.total!==null&&!a.partial).map(a=>({id:a.n.id,label:a.n.label,pc:a.total/a.n.dv*100,budget:A_BUDGET.has(a.n.id)})),n=(a,s)=>a.pc-s.pc;return{short:t.filter(a=>!a.budget&&a.pc<50).sort(n),over:t.filter(a=>!a.budget&&a.pc>=100).sort((a,s)=>-n(a,s)),budget:t.filter(a=>a.budget&&a.pc>=100).sort((a,s)=>-n(a,s))}}const DAY_TABS=[["inputs","#dayTabInputs","#dayInputsContainer"],["day","#dayTabDay","#daySumContainer"],["totals","#dayTabTotals","#dayTotalsContainer"]],dayUnseen=new Set;function dayChanged(){for(const[e]of DAY_TABS)e!==S.dayTab&&dayUnseen.add(e);savePrefs(),renderDayDots()}function renderDayDots(){for(const[e,t]of DAY_TABS)$(`${t} .tdot`).hidden=!dayUnseen.has(e)}function addToDay(e,t=DEFAULT_G){if(!BY_SLUG.has(e))return null;const n=S.day.find(s=>s.slug===e),a=n||{slug:e,g:0};return a.g=clampG(a.g+t),n||S.day.push(a),dayChanged(),a}function setDayGrams(e,t){const n=S.day.find(s=>s.slug===e);if(!n)return;const a=n.g;if(n.g=clampG(t),n.g===a){savePrefs();return}dayChanged()}function removeFromDay(e){S.day=S.day.filter(t=>t.slug!==e),dayChanged()}function rows(){const e=S.q.trim().toLowerCase();let t=FOODS.map((s,o)=>({f:s,i:o}));S.favsOnly&&(t=t.filter(s=>isFav(s.i))),S.cat&&(t=t.filter(s=>s.f.cat===S.cat)),e&&(t=t.filter(s=>(s.f.name+" "+(s.f.alt||"")+" "+s.f.state+" "+s.f.cat).toLowerCase().includes(e)));const{id:n,dir:a}=S.sort;return t.sort((s,o)=>{if(n==="__name")return a*s.f.name.localeCompare(o.f.name);const i=nut(n),d=i.evidence?evSortKey(slugAt(s.i),i.id):shown(s.f,i),r=i.evidence?evSortKey(slugAt(o.i),i.id):shown(o.f,i);return d===r?s.f.name.localeCompare(o.f.name):d===null?1:r===null?-1:a*(d-r)}),t}function renderGroups(){const e={};NUTS.forEach(n=>e[n.group]=(e[n.group]||0)+1);const t={};cols().forEach(n=>t[n.group]=(t[n.group]||0)+1),$("#groupNav").innerHTML=GROUPS.map(n=>{const a=t[n.id]??0,s=e[n.id]??0,o=a>0&&a<s;return`
    <li><button class="navbtn" type="button" data-grp="${n.id}"
        aria-pressed="${a>0}"${o?` title="${a} of the ${s} ${n.label.toLowerCase()} columns are showing"`:""}>
      ${n.icon}<span>${n.label}</span>
      <span class="count">${o?`${a}/${s}`:s}</span>
      ${o?`<span class="sr">, ${a} of ${s} showing</span>`:""}
      <span class="dot"></span></button></li>`}).join("")}function renderCats(){const e={};FOODS.forEach(n=>e[n.cat]=(e[n.cat]||0)+1);const t=[["","All foods",FOODS.length],...CATS.map(n=>[n,n,e[n]])];$("#catNav").innerHTML=t.map(([n,a,s])=>`
    <li><button class="navbtn sub" type="button" data-cat="${esc(n)}"
        aria-pressed="${S.cat===n}">
      <span>${esc(a)}</span>
      <span class="count">${s}</span><span class="dot"></span></button></li>`).join("")}function setCat(e){S.cat=e===S.cat?"":e,renderCats(),say(S.cat?`Showing ${S.cat} only.`:"Showing all categories."),savePrefs(),render()}function toggleGroup(e){S.lens&&(S.groups=new Set(cols().map(n=>n.group)),S.lens="",renderLensSelect()),S.groups.has(e)?S.groups.delete(e):S.groups.add(e);const t=!S.groups.size;t&&S.groups.add("macro"),renderGroups(),$(`#groupNav [data-grp="${e}"]`).focus(),say(t?"The table needs at least one group, so macronutrients stay shown.":`${groupOf(e).label} ${S.groups.has(e)?"shown":"hidden"}. ${cols().length} nutrient columns visible.`),savePrefs(),render()}const LENS_ADD="__add";function renderLensSelect(){const e=t=>`<option value="${esc(t.id)}"${t.id===S.lens?" selected":""}${t.why?` title="${esc(t.why)}"`:""}>${esc(t.name)}</option>`;$("#lensSel").innerHTML=`<option value=""${S.lens?"":" selected"}>None</option><optgroup label="Built in">${BUILTIN_LENSES.map(e).join("")}</optgroup>`+(S.custom.length?`<optgroup label="Yours">${S.custom.map(e).join("")}</optgroup>`:"")+`<option value="${LENS_ADD}" title="Build your own preset from any nutrients.">Add…</option>`,$("#lensSel").classList.toggle("lensactive",!!S.lens),renderLensNote()}function renderLensNote(){const e=lensById(S.lens),t=$("#lensNote");if(!e){t.hidden=!0,t.innerHTML="";return}const n=e.ids.map(a=>nutOpt(a)?.label).filter(a=>a!==void 0);t.hidden=!1,t.innerHTML=`<b>${esc(e.name)}</b>`+(e.why?` ${esc(e.why)}`:` Showing ${n.length} nutrients.`)+`<span class="cols">${n.map(esc).join(" · ")}</span>`}function setLens(e){S.lens=lensById(e)?e:"";const t=lensById(S.lens);t?t.id==="all"?say(`Applying preset ${t.name}. Showing all columns.`):say(`Applying preset ${t.name}, ${t.ids.length} columns.`):say("Preset cleared."),renderLensSelect(),renderGroups(),savePrefs(),render()}let hoverNut=null;function renderNutNote(){const e=hoverNut||(S.sort.id!=="__name"?S.sort.id:null),t=e?nutOpt(e):null;$("#nutNote").innerHTML=t&&t.why?`<b>${esc(t.label)}</b> ${esc(t.why)}${gapLine(t.id)}${absorptionLine(t.id)}`:`Point at a column header, or tab onto one, to read what that nutrient
    ...
    <button class="absorbmore" type="button" data-dlg="bio">Absorption</button></span>`}const nutOf=e=>{const t=targetAnyEl(e)?.closest("[data-sort]")?.dataset.sort;return t&&t!=="__name"?t:null};function previewNut(e){e!==hoverNut&&(hoverNut=e,renderNutNote())}$("#thead").addEventListener("mouseover",e=>previewNut(nutOf(e))),$("#thead").addEventListener("mouseleave",()=>previewNut(null)),$("#thead").addEventListener("focusin",e=>previewNut(nutOf(e))),$("#thead").addEventListener("focusout",()=>previewNut(null));const SEASONINGS="Herbs & Spices",seasoningPortion=(e,t)=>{if(e.cat!==SEASONINGS)return null;const n=portionsFor(t);return n.length?n.reduce((a,s)=>s.g>a.g?s:a):null};function layout(){const e=cols();return e.map((t,n)=>{const a=e[n-1];return{...t,gstart:!a||a.group!==t.group,sorted:S.sort.id===t.id}})}const colClass=e=>[e.gstart&&"gstart",e.sorted&&"sorted"].filter(Boolean).join(" ");function renderTable(e){const t=layout(),n=e,a=S.sort.id==="__name",s=[];for(const l of t){const f=s[s.length-1];f&&f.id===l.group?f.span++:s.push({id:l.group,span:1})}const o=s.map(l=>`<th class="grp" data-g="${l.id}" colspan="${l.span}"
      scope="colgroup"><span class="grplabel">${esc(groupOf(l.id).label)}</span></th>`).join(""),i=t.map(l=>{const f=l.sorted?S.sort.dir===1?"ascending":"descending":"none",h=S.dv&&l.dv?"%DV":l.unit;return`<th scope="col" aria-sort="${f}" data-g="${l.group}" class="${colClass(l)}">
      <button class="sortbtn" type="button" data-sort="${l.id}"
        ${l.why?`title="${esc(l.why)}" aria-describedby="why-${esc(l.id)}"`:""}>
        <span>${esc(l.label)} <span class="unit">${h}</span></span>
        <span class="ar" aria-hidden="true">${l.sorted?S.sort.dir===1?I.up:I.down:I.sortable}</span>
      </button>${l.why?`<span class="sr" id="why-${esc(l.id)}">${esc(l.why)}</span>`:""}</th>`}).join("");$("#thead").innerHTML=`
    <tr>
      ...
      ${o}
    </tr><tr>${i}</tr>`;const d=new Set,r=new Set;$("#tbody").innerHTML=n.length?n.map(({f:l,i:f})=>{const h=S.basis==="kcal"?gramsPer100kcal(l):null,b=seasoningPortion(l,slugAt(f));return`
    <tr data-i="${f}" ${S.sel===f?'aria-selected="true"':""}>
      <td class="food${a?" sorted":""}"><div class="fcell">
        <span class="sw" style="--c:${l.colour}" aria-hidden="true"></span>
        <button class="fname" type="button" data-pick="${f}" data-name="${esc(l.name)}">
          <b>${esc(l.name)}${l.alt?` <span class="alt">(${esc(l.alt)})</span>`:""}</b>
          ${l.state?`<span>${esc(l.state)}</span>`:""}
          ${h===null?"":`<span class="per100">${Math.round(h)} g</span>
               <span class="sr">makes 100 kcal</span>`}
          ${b===null?"":`<span class="scale">${esc(b.label)} = ${Math.max(1,Math.round(b.g))} g</span>
               <span class="sr">, against the 100 g every figure here is measured on</span>`}
          <span class="sr">, show full profile</span></button>
        <button class="fav" type="button" data-fav="${f}" aria-pressed="${isFav(f)}">
          ${isFav(f)?I.heartFull:I.heart}
          <span class="sr">${isFav(f)?"Remove":"Add"} ${esc(l.name)} ${isFav(f)?"from":"to"} favourites</span>
        </button></div></td>
      ${t.map(m=>{if(m.evidence){if(S.dv)return`<td class="num ${colClass(m)}" data-g="${m.group}" data-n="${esc(m.id)}" data-ev="nodv"><span class="noref" aria-hidden="true">&ndash;</span><span class="sr">no daily value published</span></td>`;const w=ev(slugAt(f),m.id),k=evGrade(slugAt(f),w)==="proxy"?' data-match="proxy"':"";return w?.state==="estimated"&&r.add("calc"),k&&r.add("proxy"),`<td class="num ${colClass(m)}" data-g="${m.group}" data-n="${esc(m.id)}" data-ev="${w?w.state:"none"}"${k}>${esc(evText(w,m.dp))}</td>`}const g=shown(l,m),v=g===0||g===null,y=g===null?null:noteFor(f,m.id);return y&&d.add(y),`<td class="num${v?" low":""} ${colClass(m)}" data-g="${m.group}" data-n="${esc(m.id)}">${fmt(g,m)}${y?noteMark(y):""}</td>`}).join("")}
    </tr>`}).join(""):`<tr><td class="empty" colspan="${t.length+1}">${emptyState()}</td></tr>`;const p=lensById(S.lens);$("#cap").textContent=(n.length===FOODS.length?`${FOODS.length} vegan foods`:`Showing ${n.length} of ${FOODS.length} vegan foods`)+(t.length===NUTS.length?`, all ${t.length} nutrient columns`:`, ${t.length} of ${NUTS.length} nutrient columns`)+`. Values ${basisLabel()} of food`+(S.dv?", shown as % of adult daily value.":".")+(S.favsOnly?" Favourites only.":"")+(p?` ${p.name} preset applied.`:"")+(S.dv&&S.basis==="kcal"?" 5% here is a full day's worth at 2000 kcal.":"");const c={calc:["calc","Calculated by the source","The database published this as a calculation or an inference from a similar food rather than as an assay of this one. Japan's tables print such figures in parentheses."],proxy:["~","Measured in a proxy food","The figure itself was measured. What is only approximate is the food: the closest row in that database was a related species or a notably different preparation, so it stands in for the one named here."]},u=$("#noteKey");u.hidden=!d.size&&!r.size,u.innerHTML=[...r].map(l=>{const[f,h,b]=c[l];return`<span><sup class="fnote">${esc(f)}</sup> <b>${esc(h)}.</b> ${esc(b)}</span>`}).concat([...d].map(l=>`<span><sup class="fnote">${esc(l.marker)}</sup> <b>${esc(l.short)}.</b>
     ${esc(l.text)}</span>`)).join(""),syncHeadOffset()}function syncHeadOffset(){const e=$("#thead").rows[0];if(!e)return;const t=e.getBoundingClientRect().height;t&&$("#grid").style.setProperty("--head1",`${Math.floor(t)}px`);const n=e.cells[0],a=n&&n.getBoundingClientRect().width;a&&$("#grid").style.setProperty("--foodw",`${a}px`);const s=$("#scroller").clientWidth;s&&$("#grid").style.setProperty("--scrollw",`${s}px`)}addEventListener("resize",syncHeadOffset);function emptyState(){return S.favsOnly&&!S.favs.size?`<b>No favourites yet</b>Star a food with the heart button to build a shortlist,
      ...
      <dd>${n.flip?`1 : ${n.a.toFixed(1)}`:`${n.a.toFixed(1)} : 1`}</dd></div>`);const o=t?t.score>=100?"Meets the adult FAO/WHO pattern for every essential amino acid.":`Scored against the FAO/WHO adult pattern. ${esc(t.limiting)} caps the
           score; pairing this food with one richer in it raises the total.`:"";return`<h4 style="margin-top:18px">Protein quality</h4><dl>${s}</dl>`+(o?`<p style="font-size:11.5px;color:var(--faint);margin:8px 0 0;line-height:1.4">${o}</p>`:"")}function renderDetail(){const e=foodAt(S.sel),t=r=>shown(e,nut(r)),n=r=>evText(ev(slugAt(S.sel),r),nut(r).dp),a=S.day.find(r=>r.slug===slugAt(S.sel)),s=["fats","vitamin","mineral","carbdetail","acids","amino","plant","other"],o=[["overview","Overview",I.macro],...s.map(r=>groupOf(r)).map(r=>[r.id,r.label,r.icon]),["absorption","Absorption",I.eye]],i=new Set;let d;if(S.tab==="overview"){const r=["kcal","protein","carbs","fiber","solfibre","insolfibre","resstarch","fat","satfat"],p=NUTS.filter(c=>c.dv!==null&&c.dv>0&&c.group!=="macro").flatMap(c=>{const u=t(c.id);return u===null?[]:[{n:c,pc:u/c.dv*100}]}).filter(c=>c.pc>0).sort((c,u)=>u.pc-c.pc).slice(0,6);d="<h4>Macronutrients</h4><dl>"+r.map(c=>{const u=nut(c),l=c==="fiber"||c==="satfat"||!!u.evidence;if(u.evidence){const b=ev(slugAt(S.sel),c),m=b&&(b.state==="measured"||b.state==="range"||b.state==="estimated");return`<div class="drow sub"><dt>${esc(u.label)}</dt>
          <dd>${m?`${esc(n(c))} ${esc(u.unit)}`:`<span class="nodata">${esc(n(c))}</span>`}</dd></div>`}const f=t(c),h=c==="kcal"&&f!==null?` <span class="pc">· ${Math.round(f*4.184)} kJ</span>`:"";return`<div class="drow${l?" sub":""}"><dt>${esc(u.label)}</dt>
        <dd>${f===null?'<span class="nodata">not measured</span>':`${f.toFixed(u.dp)} ${u.unit}${h}`}</dd></div>`}).join("")+"</dl>"+proteinQualityBlock(e)+'<h4 style="margin-top:18px">Top nutrients</h4><dl>'+p.map(({n:c,pc:u})=>{const l=noteFor(S.sel,c.id);return l&&i.add(l),`<div class="drow"><dt>${esc(c.label)}</dt>
          <dd class="pc">${Math.round(u)}% DV${l?noteMark(l):""}</dd></div>`}).join("")+"</dl>"}else if(S.tab==="absorption"){const r=sourceOf(e),p=VNUTS.flatMap(u=>{const l=val(e,u.id)===null?null:noteFor(S.sel,u.id);return l&&ABSORB_NOTES.has(l.id)?[{n:u,note:l}]:[]});for(const{note:u}of p)i.add(u);const c=preparing(slugAt(S.sel));d=(c.length?"<h4>Preparation</h4>"+c.map(u=>{const l=prepMeasured(u,slugAt(S.sel));return`
        <div class="biorow ${u.direction}${l?"":" carried"}">
          <div class="biohead">
            <span class="bioarrow" aria-hidden="true">${u.direction==="up"?"↑":"↓"}</span>
            <b>${esc(agentLabel(u.agent))}</b>
            <span class="biowhen">${esc(nut(u.component).label)}</span>
          </div>
          <p>${esc(u.text)}</p>
          ${l?"":`<p class="carriednote">Measured in
            ${esc(andList(u.measuredIn.flatMap(f=>{const h=foodBySlug(f);return h?[fullName(h)]:[]})))}, not in this food. Every brassica works this way, but nobody has run
            the experiment on this one.</p>`}
        </div>`}).join("")+`<p class="nodatanote">What has to happen before there is anything to absorb.
         Nothing here adjusts a figure: the table still says what was measured in the food.
         <button class="absorbmore" type="button" data-dlg="prep">Sources</button></p>`:"")+"<h4>Absorption</h4>"+(p.length?p.map(({n:u,note:l})=>`
        <div class="biorow curated">
          <div class="biohead"><b>${esc(u.label)}</b>
            <span class="biowhen">this food</span></div>
          <p>${esc(l.text)}</p>
        </div>`).join(""):"")+(r.length?r.map(u=>{const l=nut(u),f=affecting(u).map(h=>`
          <div class="biorow ${h.direction}">
            <div class="biohead">
              <span class="bioarrow" aria-hidden="true">${h.direction==="up"?"↑":"↓"}</span>
              <b>${esc(agentLabel(h.agent))}</b>
              <span class="biowhen">${esc(h.when)}</span>
            </div>
            <p>${esc(h.text)}</p>
          </div>`).join("");return`<h5 class="biofor">${esc(l.label)}</h5>${f}`}).join(""):`<p class="nodatanote">Nothing on record applies to this food. That means no
       ...
       <button class="absorbmore" type="button" data-dlg="bio">Sources</button></p>`}else{const r=NUTS.filter(h=>h.group===S.tab),p=r.filter(h=>!h.evidence),c=p.map(h=>t(h.id)).filter(h=>h!==null),u=Math.max(...c,1e-4),l=lensIds(),f=p.length-c.length;d=`<h4>${esc(GROUPS.find(h=>h.id===S.tab)?.label||S.tab)}</h4>
      <dl>`+r.map(h=>{if(h.evidence){const y=ev(slugAt(S.sel),h.id),w=y&&(y.state==="measured"||y.state==="range"||y.state==="estimated"),k=y?.sources?.length?` <span class="pc">· ${esc(y.sources.map(T=>SRCS[T]?.short??T).join(", "))}</span>`:"";return`<div class="drow${l.has(h.id)?" lensrow":""}" style="display:block">
            <div style="display:flex;justify-content:space-between;gap:10px">
              <dt>${esc(h.label)}</dt>
              <dd>${w?`${esc(evText(y,h.dp))} ${esc(h.unit)}${k}`:`<span class="nodata">${esc(evText(y,h.dp))}</span>`}</dd>
            </div>
          </div>`}const b=t(h.id),m=b===null,g=!m&&h.dv?Math.round(b/h.dv*100):null,v=m?null:noteFor(S.sel,h.id);return v&&i.add(v),`<div class="drow${l.has(h.id)?" lensrow":""}" style="display:block">
          <div style="display:flex;justify-content:space-between;gap:10px">
            <dt>${esc(h.label)}</dt>
            <dd>${m?'<span class="nodata">not measured</span>':`${b.toFixed(h.dp)} ${h.unit}${g!==null?` <span class="pc">· ${g}%</span>`:""}${v?noteMark(v):""}`}</dd>
          </div>
  ...
  <form method="dialog">
    <div class="fhead" style="--c:${e.colour}">
      ...
      <div class="fid">
        ...
        <div class="per">${[e.state&&esc(e.state),esc(e.cat),basisLabel(),(()=>{const r=seasoningPortion(e,slugAt(S.sel));return r&&`${esc(r.label)} = ${Math.max(1,Math.round(r.g))} g`})()].filter(Boolean).map(r=>`<span>${r}</span>`).join(" · ")}</div>
      </div>
      <div class="facts">
      ...
      <div class="tabs" role="tablist" aria-label="Nutrient detail sections">
        ${o.map(([r,p,c])=>`
          <button type="button" role="tab" data-tab="${r}" id="tab-${r}"
            ...
            tabindex="${S.tab===r?0:-1}">${c}<span>${p}</span></button>`).join("")}
      </div>
      <div class="dbody" id="tabp" role="tabpanel" aria-labelledby="tab-${S.tab}" tabindex="0">${d}${[...i].map(r=>`<p class="nodatanote"><sup class="fnote">${esc(r.marker)}</sup>
          <b>${esc(r.short)}.</b> ${esc(r.text)}</p>`).join("")}</div>
      <div class="dfoot">% DV uses general adult reference values. Yours may differ.</div>
    </div>
  </form>`}function renderCounts(){$("#favCount").textContent=S.favs.size?String(S.favs.size):"",$("#dayCount").textContent=dayEntries().length?String(dayEntries().length):""}const fmtTotal=(e,t)=>e===null?"not measured":`${e.toFixed(t.dp)} ${t.unit}`,portionsFor=e=>P[e]??[];function portionSelect(e,t,n){const a=portionsFor(e);if(!a.length)return"";const s=a.findIndex(o=>clampG(o.g)===n);return`<select data-dayportion="${esc(e)}"
    ...
    </div>`}const FAO_SCORED=new Set(FAO_PATTERN.flatMap(e=>e.ids)),NON_ESSENTIAL=NUTS.filter(e=>e.group==="amino"&&!FAO_SCORED.has(e.id)).length;function aminoRefsByNutrient(e){const t=new Map;for(const n of dayAminoAcids(e))for(const a of n.ids)t.set(a,{...n,partners:n.ids.filter(s=>s!==a)});return t}function renderDayTotals(e){const t=dayContributors(),n=$("#dayTotals");if(!t.length){n.innerHTML="";return}const a=aminoRefsByNutrient(e),s=new Set,o=GROUPS.filter(i=>S.groups.has(i.id)).map(i=>{const d=e.filter(p=>p.n.group===i.id).map(p=>{const{n:c,total:u,partial:l,from:f,of:h,notes:b}=p,m=a.get(c.id),g=c.dv&&u!==null?u/c.dv*100:m?m.pc:null,v=m&&m.partners.length?`<span class="qual">with ${m.partners.map(y=>nut(y).label.toLowerCase()).join(" and ")}</span>`:"";return b.forEach(y=>s.add(y)),`<div class="totrow${u===null?" none":""}">
        <span class="totname">${esc(c.label)}${b.map(noteMark).join("")}${v}</span>
        <span class="totval">${u===null?'<span class="nodata">not measured</span>':`${u.toFixed(c.dp)} <span class="u">${esc(c.unit)}</span>`}</span>
        <span class="totbar" aria-hidden="true">${g===null?"":`<i class="${g>=100?"full":""}" style="width:${Math.min(g,100).toFixed(1)}%"></i>`}</span>
        <span class="totpc">${g===null?`<span class="noref" aria-hidden="true">&ndash;</span>
             <span class="sr">no ${c.group==="amino"?"published requirement":"daily value published"}</span>`:`${Math.round(g)}%`}</span>
        <span class="totcov">${l?`from ${f} of ${h}`:""}</span>
    ...
    sidebar. A total well over 100% can still leave you short.`]];function dayPairings(){const e=dayContributors();if(e.length<1)return[];const t=new Map;for(const a of e)for(const s of sourceOf(a.f))t.set(s,[...t.get(s)||[],a.f]);const n=[];for(const a of INTERACTIONS){if(a.when!=="same meal")continue;const s=a.agent,o=s.kind==="nutrient"?t.get(s.id)?.[0]:s.kind==="food"?e.find(i=>slugAt(i.i)===s.slug)?.f:void 0;if(o)for(const i of a.affects){const d=t.get(i)?.[0];!d||d===o||n.push({x:a,nutrient:nut(i),from:d,via:o})}}return n}const aminoRows=e=>dayAminoAcids(e).map(t=>`<div class="drow"><dt>${esc(t.label)}</dt>
    <dd>${t.got===null?'<span class="nodata">not measured</span>':`${t.got.toFixed(2)} g <span class="pc">· ${Math.round(t.pc)}%</span>`}</dd></div>`).join("");function weightRow(){const e=(o,i)=>`<button type="button" data-wunit="${o}" aria-pressed="${S.wUnit===o}">${i}</button>`,t=(o,i,d,r,p)=>`<input type="number" inputmode="numeric" id="${o}" data-w value="${i}"
       min="0"${r?` max="${r}"`:""} step="1" aria-label="Body weight in ${p}">
     <span class="u">${d}</span>`,{st:n,lb:a}=kgToStLb(S.kg),s=S.wUnit==="stlb"?t("dayStones",n,"st",40,"stones")+t("dayPounds",a,"lb",null,"pounds"):t("dayKg",+S.kg.toFixed(1),"kg",250,"kilograms");return`<div class="kgrow">
    ...
    <b>Worth pairing</b>
    <ul>${a.map(({x:g,nutrient:v,from:y,via:w})=>`<li class="${g.direction}">
      <span aria-hidden="true">${g.direction==="up"?"↑":"↓"}</span>
      ${esc(w.name)} ${g.direction==="up"?"could help":"could hold back"}
    ...
    whether any of these were eaten together.</span></div>`:"")+DAY_NOTES.map(([g,v])=>`<div><b>${esc(g)}</b> ${esc(v.replace(/\s+/g," "))}</div>`).join("")+"</div>";if(!t.length){n.innerHTML=`<div class="dhead"><h3>Your day</h3>
      <div class="per">nothing added yet</div></div>
      <div class="dbody"><p class="nodatanote" style="margin-top:0">Add a food and its
      totals appear here, in units and as a percentage of a daily value.</p>${o}</div>`;return}const i=totalOf(e,"kcal"),d=totalOf(e,"protein"),r=totalOf(e,"fiber"),p=[i,d,r].map(g=>{const v=g.n.dv&&g.total!==null?Math.round(g.total/g.n.dv*100):null,y=g.partial?` <span class="cov">from ${g.from} of ${g.of}</span>`:"";return`<div class="drow"><dt>${esc(g.n.label)}</dt>
      <dd>${fmtTotal(g.total,g.n)}${v===null?"":` <span class="pc">· ${v}%</span>`}${y}</dd></div>`}).join(""),c=dayProteinQuality(e),l=["ala","la"].every(g=>!totalOf(e,g).partial)?omegaRatio({v:e.map(g=>g.total)}):null,{short:f,over:h,budget:b}=dayStanding(e),m=g=>`<button class="jump" type="button" data-daysort="${esc(g.id)}">
    <span>${esc(g.label)}</span><b>${Math.round(g.pc)}%</b>
    ...
    <div class="dbody">
      ...
      ${c?`<dl>
        <div class="drow"><dt>Amino acid score</dt>
          <dd class="${c.score>=100?"pc":""}">${c.score}%</dd></div>
        <div class="drow"><dt>${c.score>=100?"Lowest relative to need":"Limiting amino acid"}</dt>
          <dd>${esc(c.limiting)}</dd></div>
        ${c.perKcal!==null?`<div class="drow"><dt>Protein per 100 kcal</dt>
          <dd>${c.perKcal.toFixed(1)} g</dd></div>`:""}
      </dl>
      <p class="nodatanote">${c.score>=100?`Across the whole day this meets the adult FAO/WHO pattern for every essential amino
           ...
           ${esc(c.limiting)} caps it, so adding something richer in that raises the whole day.`}</p>`:`<p class="nodatanote" style="margin-top:0">No score: at least one food in your day has
      ...
      ${l?`<h4 style="margin-top:18px">Omega balance</h4><dl>
        <div class="drow"><dt>Omega-6 : omega-3</dt>
          <dd>${l.flip?`1 : ${l.a.toFixed(1)}`:`${l.a.toFixed(1)} : 1`}</dd></div></dl>`:""}
      ...
      ${h.length?`<h4 style="margin-top:18px">Comfortable</h4>
        <div class="jumps">${h.slice(0,8).map(m).join("")}</div>`:""}
...
`)],{type:"text/csv;charset=utf-8"}),a=Object.assign(document.createElement("a"),{href:URL.createObjectURL(n),download:t});a.click(),URL.revokeObjectURL(a.href)}function csvTable(){const e=cols(),t=rows(),n=csvQuote,a=basisLabel(),o=[["Food","Also known as","State","Category",...e.map(i=>i.evidence?`${i.label} (${i.unit} per 100 g, evidence)`:`${i.label} (${S.dv&&i.dv?"%DV":i.unit} ${a})`)].map(n).join(",")].concat(t.map(({f:i,i:d})=>[n(i.name),n(i.alt||""),n(i.state),n(i.cat),...e.map(r=>{if(r.evidence)return n(evText(ev(slugAt(d),r.id),r.dp));const p=shown(i,r);return p===null?"":S.dv&&r.dv?Math.round(p/r.dv*100):p})].join(",")));download(o,"vegan-nutrients.csv"),say(`Exported ${t.length} foods and ${e.length} nutrients as CSV.`)}function today(){const e=new Date,t=n=>String(n).padStart(2,"0");return`${e.getFullYear()}-${t(e.getMonth()+1)}-${t(e.getDate())}`}function csvDay(){const e=cols().filter(r=>!r.evidence),t=dayTotals(),n=dayContributors(),a=csvQuote,s=r=>totalOf(t,r),o=a(today()),d=[["Date","Food","State","Grams",...e.map(r=>`${r.label} (${r.unit})`)].map(a).join(",")];for(const{f:r,g:p}of n)d.push([o,a(r.name),a(r.state||""),p,...e.map(c=>{const u=val(r,c.id);return u===null?"":+(u*p/100).toFixed(6)})].join(","));d.push([o,a("Total"),a(""),dayGrams(),...e.map(r=>{const p=s(r.id).total;return p===null?"":+p.toFixed(6)})].join(",")),d.push([o,a("% of daily value"),a(""),"",...e.map(r=>{const p=s(r.id).total;return r.dv&&p!==null?Math.round(p/r.dv*100):""})].join(",")),d.push([o,a("Foods measured"),a(""),"",...e.map(r=>`${s(r.id).from} of ${s(r.id).of}`).map(a)].join(",")),download(d,"my-day.csv"),say(`Exported your day, ${n.length} foods and ${e.length} nutrients, as CSV.`)}const csv=()=>S.view==="day"?csvDay():csvTable();$("#csvBtn").onclick=csv;function renderNutPick(e=new Set){$("#nutPick").innerHTML=GROUPS.map(t=>{const n=NUTS.filter(a=>a.group===t.id);return`<div class="lensgroup"><h5>${esc(t.label)}</h5><div class="nutgrid">`+n.map(a=>`<label><input type="checkbox" name="nut" value="${esc(a.id)}"
      ...
      Your presets</p>`+S.custom.map(t=>`<div class="savedlens">
    ...
    ${[...e.entries()].sort((a,s)=>t(a[0])-t(s[0])).map(([a,s])=>{const o=a.split("+").map(d=>nutOpt(d)?.label??d).join(", ").replace(/, ([^,]*)$/," and $1"),i=s.map(d=>`
        <div class="biorow ${d.direction}">
          <div class="biohead">
            <span class="bioarrow" aria-hidden="true">${d.direction==="up"?"↑":"↓"}</span>
            <b>${esc(agentLabel(d.agent))}</b>
            <span class="biowhen">${esc(d.when)}</span>
          </div>
          <p>${esc(d.text)}</p>
          ${d.cites.map(r=>`<cite>${esc(X.sources[r]??r)}</cite>`).join("")}
    ...
    foods.</p>`}function prepDialog(){const e=new Map;for(const n of PREP.preparations)e.set(n.component,[...e.get(n.component)||[],n]);return`
    ...
    ${[...e.entries()].sort((n,a)=>(IDX.get(n[0])??999)-(IDX.get(a[0])??999)).map(([n,a])=>{const s=a.map(o=>{const i=o.applies.flatMap(d=>{const r=foodBySlug(d);return r?[fullName(r)]:[]});return`
        <div class="biorow ${o.direction}">
          <div class="biohead">
            <span class="bioarrow" aria-hidden="true">${o.direction==="up"?"↑":"↓"}</span>
            <b>${esc(agentLabel(o.agent))}</b>
            <span class="biowhen">${o.applies.length} ${o.applies.length===1?"food":"foods"}</span>
          </div>
          <p>${esc(o.text)}</p>
          <p class="carriednote">Shown on ${esc(andList(i))}.
            ${o.measuredIn.length<o.applies.length?`Measured in ${esc(andList(o.measuredIn.flatMap(d=>{const r=foodBySlug(d);return r?[fullName(r)]:[]})))}; carried over to the rest.`:"Measured in every food it is shown on."}</p>
          ${o.cites.map(d=>`<cite>${esc(PREP.sources[d]??d)}</cite>`).join("")}
        </div>`}).join("");return`<h4>${esc(nutOpt(n)?.label??n)}</h4>${s}`}).join("")}
    <h4>What is not here</h4>
    ...
    enzyme is intact. There is nothing to do.</p>`}function gapsDialog(){const e=n=>G.gaps.filter(a=>a.tier===n),t=n=>`
