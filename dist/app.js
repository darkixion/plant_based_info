"use strict";const NUTS=DATA.nutrients,FOODS=DATA.foods,GROUPS=[{id:"macro",label:"Macronutrients",icon:I.macro},{id:"fats",label:"Omega & fats",icon:I.fats},{id:"amino",label:"Amino acids",icon:I.amino},{id:"vitamin",label:"Vitamins",icon:I.vit},{id:"mineral",label:"Minerals",icon:I.min},{id:"carbdetail",label:"Carbohydrate detail",icon:I.carb},{id:"acids",label:"Organic acids",icon:I.acid},{id:"plant",label:"Plant compounds",icon:I.plant},{id:"other",label:"Anti-nutrients",icon:I.other}],VNUTS=NUTS.filter(e=>!e.evidence),IDX=new Map(VNUTS.map((e,t)=>[e.id,t])),CATS=[...new Set(FOODS.map(e=>e.cat))].sort(),slugify=e=>`${e.name} ${e.state||""}`.toLowerCase().trim().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,""),SLUGS=FOODS.map(slugify),BY_SLUG=new Map(SLUGS.map((e,t)=>[e,t])),BY_ID=new Map(NUTS.map(e=>[e.id,e])),nutOpt=e=>BY_ID.get(e),nut=e=>{const t=nutOpt(e);if(!t)throw new Error(`unknown nutrient id: ${e}`);return t},foodAt=e=>{const t=FOODS[e];if(!t)throw new Error(`no food at index ${e}`);return t},slugAt=e=>{const t=SLUGS[e];if(t===void 0)throw new Error(`no food at index ${e}`);return t},foodBySlug=e=>{const t=BY_SLUG.get(e);return t===void 0?void 0:FOODS[t]},groupOf=e=>{const t=GROUPS.find(n=>n.id===e);if(!t)throw new Error(`unknown nutrient group: ${e}`);return t},isGroup=e=>GROUPS.some(t=>t.id===e),RENAMED={"navy-beans-cooked":"haricot-beans-cooked"},currentSlug=e=>RENAMED[e]||e,NOTES=DATA.notes||[],NOTE_AT=new Map;for(const e of NOTES)for(const[t,n]of Object.entries(e.cells||{}))for(const a of n)NOTE_AT.set(`${t} ${a}`,e);const noteFor=(e,t)=>NOTE_AT.get(`${slugAt(e)} ${t}`)||null,ABSORB_NOTES=new Set(["oxalate-high","oxalate-low"]),noteMark=e=>`<sup class="fnote" aria-hidden="true">${esc(e.marker)}</sup><span class="sr">, ${esc(e.short)}</span>`,INTERACTIONS=X.interactions,AFFECTING=new Map;for(const e of INTERACTIONS)for(const t of e.affects)AFFECTING.set(t,[...AFFECTING.get(t)||[],e]);const ACTING=new Map;for(const e of INTERACTIONS)e.agent.kind==="nutrient"&&ACTING.set(e.agent.id,[...ACTING.get(e.agent.id)||[],e]);const affecting=e=>AFFECTING.get(e)||[],acting=e=>ACTING.get(e)||[],hasInteractions=e=>affecting(e).length>0||acting(e).length>0,affectedLabels=e=>e.affects.map(t=>nutOpt(t)?.label??t),RANK_DEPTH=10;function sourceOf(e){const t=[];for(const n of VNUTS){if(!hasInteractions(n.id))continue;const a=val(e,n.id);if(!(a===null||a<=0))if(n.dv)a/n.dv>=.1&&t.push(n.id);else{const s=IDX.get(n.id);if(s===void 0)continue;let o=0;for(const i of FOODS){const c=i.v[s];if(typeof c=="number"&&c>a&&++o>=RANK_DEPTH)break}o<RANK_DEPTH&&t.push(n.id)}}return t}function gapEvidence(e){const t=nutOpt(e);if(!t||t.evidence)return null;let n=0,a=0,s=0,o=0,i=null,c=null;return FOODS.forEach((r,d)=>{const l=val(r,e);if(l===null){s++;return}if(l===0){a++;return}if(n++,noteFor(d,e)?.id==="fortified"){o++;return}if(t.dv){const u=l/t.dv*100;(i===null||u>i)&&(i=u,c=r.name)}}),{above:n,zero:a,unassayed:s,of:FOODS.length,fortified:o,bestPc:i,bestFood:c}}function gapEvidenceText(e){const t=e.nutrients.map(n=>{const a=gapEvidence(n);if(!a)return"";const s=nutOpt(n)?.label??n,o=[`${a.above} of ${a.of} foods ${a.above===1?"has":"have"} any at all`];return a.fortified&&o.push(`${a.fortified} of those ${a.fortified===1?"is":"are"} fortification rather than the food`),a.zero&&o.push(`${a.zero} were measured and found to contain none`),a.unassayed&&o.push(`${a.unassayed} ${a.unassayed===1?"was":"were"} never assayed`),a.bestPc!==null&&a.bestFood&&o.push(`the best unfortified is ${esc(a.bestFood)}, at ${Math.round(a.bestPc)}% of a daily value per 100 g`),`<li><b>${esc(s)}:</b> ${o.join("; ")}.</li>`}).filter(Boolean);return t.length?`<ul class="gapev"><li class="gapevhead">Measured from this table,
    ...
    <button class="absorbmore" type="button" data-dlg="bio">Absorption</button></span>`}const nutOf=e=>{const t=targetAnyEl(e)?.closest("[data-sort]")?.dataset.sort;return t&&t!=="__name"?t:null};function previewNut(e){e!==hoverNut&&(hoverNut=e,renderNutNote())}$("#thead").addEventListener("mouseover",e=>previewNut(nutOf(e))),$("#thead").addEventListener("mouseleave",()=>previewNut(null)),$("#thead").addEventListener("focusin",e=>previewNut(nutOf(e))),$("#thead").addEventListener("focusout",()=>previewNut(null));function layout(){const e=cols();return e.map((t,n)=>{const a=e[n-1];return{...t,gstart:!a||a.group!==t.group,sorted:S.sort.id===t.id}})}const colClass=e=>[e.gstart&&"gstart",e.sorted&&"sorted"].filter(Boolean).join(" ");function renderTable(e){const t=layout(),n=e,a=S.sort.id==="__name",s=GROUPS.filter(d=>S.groups.has(d.id)).map(d=>{const l=t.filter(u=>u.group===d.id);return`<th class="grp" data-g="${d.id}" colspan="${l.length}"
      ...
      <button class="sortbtn" type="button" data-sort="${d.id}"
        ...
        <span>${esc(d.label)} <span class="unit">${u}</span></span>
    ...
    <tr data-i="${l}" ${S.sel===l?'aria-selected="true"':""}>
      ...
      ${t.map(p=>{if(p.evidence){if(S.dv)return`<td class="num ${colClass(p)}" data-g="${p.group}" data-n="${esc(p.id)}" data-ev="nodv"><span class="noref" aria-hidden="true">&ndash;</span><span class="sr">no daily value published</span></td>`;const m=ev(slugAt(l),p.id),h=m&&evFood(slugAt(l))?.match==="proxy"?' data-match="proxy"':"";return`<td class="num ${colClass(p)}" data-g="${p.group}" data-n="${esc(p.id)}" data-ev="${m?m.state:"none"}"${h}>${esc(evText(m,p.dp))}</td>`}const g=shown(d,p),f=g===0||g===null,b=g===null?null:noteFor(l,p.id);return b&&i.add(b),`<td class="num${f?" low":""} ${colClass(p)}" data-g="${p.group}" data-n="${esc(p.id)}">${fmt(g,p)}${b?noteMark(b):""}</td>`}).join("")}
    </tr>`}).join(""):`<tr><td class="empty" colspan="${t.length+1}">${emptyState()}</td></tr>`;const c=lensById(S.lens);$("#cap").textContent=(n.length===FOODS.length?`${FOODS.length} vegan foods`:`Showing ${n.length} of ${FOODS.length} vegan foods`)+(t.length===NUTS.length?`, all ${t.length} nutrient columns`:`, ${t.length} of ${NUTS.length} nutrient columns`)+`. Values ${basisLabel()} of food`+(S.dv?", shown as % of adult daily value.":".")+(S.favsOnly?" Favourites only.":"")+(c?` ${c.name} preset applied.`:"")+(S.dv&&S.basis==="kcal"?" 5% here is a full day's worth at 2000 kcal.":"");const r=$("#noteKey");r.hidden=!i.size,r.innerHTML=[...i].map(d=>`<span><sup class="fnote">${esc(d.marker)}</sup> <b>${esc(d.short)}.</b>
     ${esc(d.text)}</span>`).join(""),syncHeadOffset()}function syncHeadOffset(){const e=$("#thead").rows[0];if(!e)return;const t=e.getBoundingClientRect().height;t&&$("#grid").style.setProperty("--head1",`${Math.floor(t)}px`);const n=e.cells[0],a=n&&n.getBoundingClientRect().width;a&&$("#grid").style.setProperty("--foodw",`${a}px`);const s=$("#scroller").clientWidth;s&&$("#grid").style.setProperty("--scrollw",`${s}px`)}addEventListener("resize",syncHeadOffset);function emptyState(){return S.favsOnly&&!S.favs.size?`<b>No favourites yet</b>Star a food with the heart button to build a shortlist,
      ...
      <dd>${n.flip?`1 : ${n.a.toFixed(1)}`:`${n.a.toFixed(1)} : 1`}</dd></div>`);const o=t?t.score>=100?"Meets the adult FAO/WHO pattern for every essential amino acid.":`Scored against the FAO/WHO adult pattern. ${esc(t.limiting)} caps the
           score; pairing this food with one richer in it raises the total.`:"";return`<h4 style="margin-top:18px">Protein quality</h4><dl>${s}</dl>`+(o?`<p style="font-size:11.5px;color:var(--faint);margin:8px 0 0;line-height:1.4">${o}</p>`:"")}function renderDetail(){const e=foodAt(S.sel),t=r=>shown(e,nut(r)),n=r=>evText(ev(slugAt(S.sel),r),nut(r).dp),a=S.day.find(r=>r.slug===slugAt(S.sel)),s=["fats","vitamin","mineral","carbdetail","acids","amino","plant","other"],o=[["overview","Overview",I.macro],...s.map(r=>groupOf(r)).map(r=>[r.id,r.label,r.icon]),["absorption","Absorption",I.eye]],i=new Set;let c;if(S.tab==="overview"){const r=["kcal","protein","carbs","fiber","solfibre","insolfibre","resstarch","fat","satfat"],d=NUTS.filter(l=>l.dv!==null&&l.dv>0&&l.group!=="macro").flatMap(l=>{const u=t(l.id);return u===null?[]:[{n:l,pc:u/l.dv*100}]}).filter(l=>l.pc>0).sort((l,u)=>u.pc-l.pc).slice(0,6);c="<h4>Macronutrients</h4><dl>"+r.map(l=>{const u=nut(l),p=l==="fiber"||l==="satfat"||!!u.evidence;if(u.evidence){const b=ev(slugAt(S.sel),l),m=b&&(b.state==="measured"||b.state==="range"||b.state==="estimated");return`<div class="drow sub"><dt>${esc(u.label)}</dt>
          <dd>${m?`${esc(n(l))} ${esc(u.unit)}`:`<span class="nodata">${esc(n(l))}</span>`}</dd></div>`}const g=t(l),f=l==="kcal"&&g!==null?` <span class="pc">· ${Math.round(g*4.184)} kJ</span>`:"";return`<div class="drow${p?" sub":""}"><dt>${esc(u.label)}</dt>
        <dd>${g===null?'<span class="nodata">not measured</span>':`${g.toFixed(u.dp)} ${u.unit}${f}`}</dd></div>`}).join("")+"</dl>"+proteinQualityBlock(e)+'<h4 style="margin-top:18px">Top nutrients</h4><dl>'+d.map(({n:l,pc:u})=>{const p=noteFor(S.sel,l.id);return p&&i.add(p),`<div class="drow"><dt>${esc(l.label)}</dt>
          <dd class="pc">${Math.round(u)}% DV${p?noteMark(p):""}</dd></div>`}).join("")+"</dl>"}else if(S.tab==="absorption"){const r=sourceOf(e),d=VNUTS.flatMap(l=>{const u=val(e,l.id)===null?null:noteFor(S.sel,l.id);return u&&ABSORB_NOTES.has(u.id)?[{n:l,note:u}]:[]});for(const{note:l}of d)i.add(l);c="<h4>Absorption</h4>"+(d.length?d.map(({n:l,note:u})=>`
        <div class="biorow curated">
          <div class="biohead"><b>${esc(l.label)}</b>
            <span class="biowhen">this food</span></div>
          <p>${esc(u.text)}</p>
        </div>`).join(""):"")+(r.length?r.map(l=>{const u=nut(l),p=affecting(l).map(g=>`
          <div class="biorow ${g.direction}">
            ...
            <p>${esc(g.text)}</p>
          </div>`).join("");return`<h5 class="biofor">${esc(u.label)}</h5>${p}`}).join(""):`<p class="nodatanote">Nothing on record applies to this food. That means no
       ...
       <button class="absorbmore" type="button" data-dlg="bio">Sources</button></p>`}else{const r=NUTS.filter(f=>f.group===S.tab),d=r.filter(f=>!f.evidence),l=d.map(f=>t(f.id)).filter(f=>f!==null),u=Math.max(...l,1e-4),p=lensIds(),g=d.length-l.length;c=`<h4>${esc(GROUPS.find(f=>f.id===S.tab)?.label||S.tab)}</h4>
      <dl>`+r.map(f=>{if(f.evidence){const y=ev(slugAt(S.sel),f.id),w=y&&(y.state==="measured"||y.state==="range"||y.state==="estimated"),T=y?.sources?.length?` <span class="pc">· ${esc(y.sources.map(k=>SRCS[k]?.country??k).join(", "))}</span>`:"";return`<div class="drow${p.has(f.id)?" lensrow":""}" style="display:block">
            <div style="display:flex;justify-content:space-between;gap:10px">
              <dt>${esc(f.label)}</dt>
              <dd>${w?`${esc(evText(y,f.dp))} ${esc(f.unit)}${T}`:`<span class="nodata">${esc(evText(y,f.dp))}</span>`}</dd>
            </div>
          </div>`}const b=t(f.id),m=b===null,h=!m&&f.dv?Math.round(b/f.dv*100):null,v=m?null:noteFor(S.sel,f.id);return v&&i.add(v),`<div class="drow${p.has(f.id)?" lensrow":""}" style="display:block">
          <div style="display:flex;justify-content:space-between;gap:10px">
            <dt>${esc(f.label)}</dt>
            <dd>${m?'<span class="nodata">not measured</span>':`${b.toFixed(f.dp)} ${f.unit}${h!==null?` <span class="pc">· ${h}%</span>`:""}${v?noteMark(v):""}`}</dd>
          </div>
  ...
  <form method="dialog">
    ...
    <div class="dayfoot">
      ...
      <span class="push">${e.length} food${e.length===1?"":"s"} ·
        ...
        <span class="totbar" aria-hidden="true">${h===null?"":`<i class="${h>=100?"full":""}" style="width:${Math.min(h,100).toFixed(1)}%"></i>`}</span>
        <span class="totpc">${h===null?`<span class="noref" aria-hidden="true">&ndash;</span>
             <span class="sr">no ${l.group==="amino"?"published requirement":"daily value published"}</span>`:`${Math.round(h)}%`}</span>
        <span class="totcov">${p?`from ${g} of ${f}`:""}</span>
    ...
    <b>Worth pairing</b>
    <ul>${a.map(({x:h,nutrient:v,from:y,via:w})=>`<li class="${h.direction}">
      <span aria-hidden="true">${h.direction==="up"?"↑":"↓"}</span>
      ${esc(w.name)} ${h.direction==="up"?"could help":"could hold back"}
    ...
    whether any of these were eaten together.</span></div>`:"")+DAY_NOTES.map(([h,v])=>`<div><b>${esc(h)}</b> ${esc(v.replace(/\s+/g," "))}</div>`).join("")+"</div>";if(!t.length){n.innerHTML=`<div class="dhead"><h3>Your day</h3>
      <div class="per">nothing added yet</div></div>
      <div class="dbody"><p class="nodatanote" style="margin-top:0">Add a food and its
      totals appear here, in units and as a percentage of a daily value.</p>${o}</div>`;return}const i=totalOf(e,"kcal"),c=totalOf(e,"protein"),r=totalOf(e,"fiber"),d=[i,c,r].map(h=>{const v=h.n.dv&&h.total!==null?Math.round(h.total/h.n.dv*100):null,y=h.partial?` <span class="cov">from ${h.from} of ${h.of}</span>`:"";return`<div class="drow"><dt>${esc(h.n.label)}</dt>
      <dd>${fmtTotal(h.total,h.n)}${v===null?"":` <span class="pc">· ${v}%</span>`}${y}</dd></div>`}).join(""),l=dayProteinQuality(e),p=["ala","la"].every(h=>!totalOf(e,h).partial)?omegaRatio({v:e.map(h=>h.total)}):null,{short:g,over:f,budget:b}=dayStanding(e),m=h=>`<button class="jump" type="button" data-daysort="${esc(h.id)}">
    ...
    <div class="dbody">
      ...
      ${p?`<h4 style="margin-top:18px">Omega balance</h4><dl>
        <div class="drow"><dt>Omega-6 : omega-3</dt>
          <dd>${p.flip?`1 : ${p.a.toFixed(1)}`:`${p.a.toFixed(1)} : 1`}</dd></div></dl>`:""}
