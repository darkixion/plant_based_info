"use strict";const NUTS=DATA.nutrients,FOODS=DATA.foods,GROUPS=[{id:"macro",label:"Macronutrients",icon:I.macro},{id:"fats",label:"Omega & fats",icon:I.fats},{id:"amino",label:"Amino acids",icon:I.amino},{id:"vitamin",label:"Vitamins",icon:I.vit},{id:"mineral",label:"Minerals",icon:I.min},{id:"carbdetail",label:"Carbohydrate detail",icon:I.carb},{id:"acids",label:"Organic acids",icon:I.acid},{id:"plant",label:"Plant compounds",icon:I.plant},{id:"other",label:"Anti-nutrients",icon:I.other}],VNUTS=NUTS.filter(e=>!e.evidence),IDX=new Map(VNUTS.map((e,t)=>[e.id,t])),CATS=[...new Set(FOODS.map(e=>e.cat))].sort(),slugify=e=>`${e.name} ${e.state||""}`.toLowerCase().trim().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,""),SLUGS=FOODS.map(slugify),BY_SLUG=new Map(SLUGS.map((e,t)=>[e,t])),BY_ID=new Map(NUTS.map(e=>[e.id,e])),nutOpt=e=>BY_ID.get(e),nut=e=>{const t=nutOpt(e);if(!t)throw new Error(`unknown nutrient id: ${e}`);return t},foodAt=e=>{const t=FOODS[e];if(!t)throw new Error(`no food at index ${e}`);return t},slugAt=e=>{const t=SLUGS[e];if(t===void 0)throw new Error(`no food at index ${e}`);return t},foodBySlug=e=>{const t=BY_SLUG.get(e);return t===void 0?void 0:FOODS[t]},groupOf=e=>{const t=GROUPS.find(n=>n.id===e);if(!t)throw new Error(`unknown nutrient group: ${e}`);return t},isGroup=e=>GROUPS.some(t=>t.id===e),RENAMED={"navy-beans-cooked":"haricot-beans-cooked"},currentSlug=e=>RENAMED[e]||e,NOTES=DATA.notes||[],NOTE_AT=new Map;for(const e of NOTES)for(const[t,n]of Object.entries(e.cells||{}))for(const a of n)NOTE_AT.set(`${t} ${a}`,e);const noteFor=(e,t)=>NOTE_AT.get(`${slugAt(e)} ${t}`)||null,ABSORB_NOTES=new Set(["oxalate-high","oxalate-low"]),noteMark=e=>`<sup class="fnote" aria-hidden="true">${esc(e.marker)}</sup><span class="sr">, ${esc(e.short)}</span>`,INTERACTIONS=X.interactions,AFFECTING=new Map;for(const e of INTERACTIONS)for(const t of e.affects)AFFECTING.set(t,[...AFFECTING.get(t)||[],e]);const ACTING=new Map;for(const e of INTERACTIONS)e.agent.kind==="nutrient"&&ACTING.set(e.agent.id,[...ACTING.get(e.agent.id)||[],e]);const affecting=e=>AFFECTING.get(e)||[],acting=e=>ACTING.get(e)||[],hasInteractions=e=>affecting(e).length>0||acting(e).length>0,affectedLabels=e=>e.affects.map(t=>nutOpt(t)?.label??t),RANK_DEPTH=10;function sourceOf(e){const t=[];for(const n of VNUTS){if(!hasInteractions(n.id))continue;const a=val(e,n.id);if(!(a===null||a<=0))if(n.dv)a/n.dv>=.1&&t.push(n.id);else{const s=IDX.get(n.id);if(s===void 0)continue;let o=0;for(const i of FOODS){const d=i.v[s];if(typeof d=="number"&&d>a&&++o>=RANK_DEPTH)break}o<RANK_DEPTH&&t.push(n.id)}}return t}function gapEvidence(e){const t=nutOpt(e);if(!t||t.evidence)return null;let n=0,a=0,s=0,o=0,i=null,d=null;return FOODS.forEach((l,h)=>{const r=val(l,e);if(r===null){s++;return}if(r===0){a++;return}if(n++,noteFor(h,e)?.id==="fortified"){o++;return}if(t.dv){const c=r/t.dv*100;(i===null||c>i)&&(i=c,d=l.name)}}),{above:n,zero:a,unassayed:s,of:FOODS.length,fortified:o,bestPc:i,bestFood:d}}function gapEvidenceText(e){const t=e.nutrients.map(n=>{const a=gapEvidence(n);if(!a)return"";const s=nutOpt(n)?.label??n,o=[`${a.above} of ${a.of} foods ${a.above===1?"has":"have"} any at all`];return a.fortified&&o.push(`${a.fortified} of those ${a.fortified===1?"is":"are"} fortification rather than the food`),a.zero&&o.push(`${a.zero} were measured and found to contain none`),a.unassayed&&o.push(`${a.unassayed} ${a.unassayed===1?"was":"were"} never assayed`),a.bestPc!==null&&a.bestFood&&o.push(`the best unfortified is ${esc(a.bestFood)}, at ${Math.round(a.bestPc)}% of a daily value per 100 g`),`<li><b>${esc(s)}:</b> ${o.join("; ")}.</li>`}).filter(Boolean);return t.length?`<ul class="gapev"><li class="gapevhead">Measured from this table,
    ...
    <tr>
      ...
      ${o}
    </tr><tr>${i}</tr>`;const d=new Set;$("#tbody").innerHTML=n.length?n.map(({f:r,i:c})=>{const f=S.basis==="kcal"?gramsPer100kcal(r):null,g=seasoningPortion(r,slugAt(c));return`
    <tr data-i="${c}" ${S.sel===c?'aria-selected="true"':""}>
      <td class="food${a?" sorted":""}"><div class="fcell">
        ...
        <button class="fname" type="button" data-pick="${c}" data-name="${esc(r.name)}">
          ...
          ${f===null?"":`<span class="per100">${Math.round(f)} g</span>
               <span class="sr">makes 100 kcal</span>`}
          ${g===null?"":`<span class="scale">${esc(g.label)} = ${Math.max(1,Math.round(g.g))} g</span>
      ...
      ${t.map(u=>{if(u.evidence){if(S.dv)return`<td class="num ${colClass(u)}" data-g="${u.group}" data-n="${esc(u.id)}" data-ev="nodv"><span class="noref" aria-hidden="true">&ndash;</span><span class="sr">no daily value published</span></td>`;const y=ev(slugAt(c),u.id),v=y&&evFood(slugAt(c))?.match==="proxy"?' data-match="proxy"':"";return`<td class="num ${colClass(u)}" data-g="${u.group}" data-n="${esc(u.id)}" data-ev="${y?y.state:"none"}"${v}>${esc(evText(y,u.dp))}</td>`}const m=shown(r,u),b=m===0||m===null,p=m===null?null:noteFor(c,u.id);return p&&d.add(p),`<td class="num${b?" low":""} ${colClass(u)}" data-g="${u.group}" data-n="${esc(u.id)}">${fmt(m,u)}${p?noteMark(p):""}</td>`}).join("")}
    </tr>`}).join(""):`<tr><td class="empty" colspan="${t.length+1}">${emptyState()}</td></tr>`;const l=lensById(S.lens);$("#cap").textContent=(n.length===FOODS.length?`${FOODS.length} vegan foods`:`Showing ${n.length} of ${FOODS.length} vegan foods`)+(t.length===NUTS.length?`, all ${t.length} nutrient columns`:`, ${t.length} of ${NUTS.length} nutrient columns`)+`. Values ${basisLabel()} of food`+(S.dv?", shown as % of adult daily value.":".")+(S.favsOnly?" Favourites only.":"")+(l?` ${l.name} preset applied.`:"")+(S.dv&&S.basis==="kcal"?" 5% here is a full day's worth at 2000 kcal.":"");const h=$("#noteKey");h.hidden=!d.size,h.innerHTML=[...d].map(r=>`<span><sup class="fnote">${esc(r.marker)}</sup> <b>${esc(r.short)}.</b>
     ${esc(r.text)}</span>`).join(""),syncHeadOffset()}function syncHeadOffset(){const e=$("#thead").rows[0];if(!e)return;const t=e.getBoundingClientRect().height;t&&$("#grid").style.setProperty("--head1",`${Math.floor(t)}px`);const n=e.cells[0],a=n&&n.getBoundingClientRect().width;a&&$("#grid").style.setProperty("--foodw",`${a}px`);const s=$("#scroller").clientWidth;s&&$("#grid").style.setProperty("--scrollw",`${s}px`)}addEventListener("resize",syncHeadOffset);function emptyState(){return S.favsOnly&&!S.favs.size?`<b>No favourites yet</b>Star a food with the heart button to build a shortlist,
      ...
      <dd>${n.flip?`1 : ${n.a.toFixed(1)}`:`${n.a.toFixed(1)} : 1`}</dd></div>`);const o=t?t.score>=100?"Meets the adult FAO/WHO pattern for every essential amino acid.":`Scored against the FAO/WHO adult pattern. ${esc(t.limiting)} caps the
           score; pairing this food with one richer in it raises the total.`:"";return`<h4 style="margin-top:18px">Protein quality</h4><dl>${s}</dl>`+(o?`<p style="font-size:11.5px;color:var(--faint);margin:8px 0 0;line-height:1.4">${o}</p>`:"")}function renderDetail(){const e=foodAt(S.sel),t=l=>shown(e,nut(l)),n=l=>evText(ev(slugAt(S.sel),l),nut(l).dp),a=S.day.find(l=>l.slug===slugAt(S.sel)),s=["fats","vitamin","mineral","carbdetail","acids","amino","plant","other"],o=[["overview","Overview",I.macro],...s.map(l=>groupOf(l)).map(l=>[l.id,l.label,l.icon]),["absorption","Absorption",I.eye]],i=new Set;let d;if(S.tab==="overview"){const l=["kcal","protein","carbs","fiber","solfibre","insolfibre","resstarch","fat","satfat"],h=NUTS.filter(r=>r.dv!==null&&r.dv>0&&r.group!=="macro").flatMap(r=>{const c=t(r.id);return c===null?[]:[{n:r,pc:c/r.dv*100}]}).filter(r=>r.pc>0).sort((r,c)=>c.pc-r.pc).slice(0,6);d="<h4>Macronutrients</h4><dl>"+l.map(r=>{const c=nut(r),f=r==="fiber"||r==="satfat"||!!c.evidence;if(c.evidence){const m=ev(slugAt(S.sel),r),b=m&&(m.state==="measured"||m.state==="range"||m.state==="estimated");return`<div class="drow sub"><dt>${esc(c.label)}</dt>
          <dd>${b?`${esc(n(r))} ${esc(c.unit)}`:`<span class="nodata">${esc(n(r))}</span>`}</dd></div>`}const g=t(r),u=r==="kcal"&&g!==null?` <span class="pc">· ${Math.round(g*4.184)} kJ</span>`:"";return`<div class="drow${f?" sub":""}"><dt>${esc(c.label)}</dt>
        <dd>${g===null?'<span class="nodata">not measured</span>':`${g.toFixed(c.dp)} ${c.unit}${u}`}</dd></div>`}).join("")+"</dl>"+proteinQualityBlock(e)+'<h4 style="margin-top:18px">Top nutrients</h4><dl>'+h.map(({n:r,pc:c})=>{const f=noteFor(S.sel,r.id);return f&&i.add(f),`<div class="drow"><dt>${esc(r.label)}</dt>
          <dd class="pc">${Math.round(c)}% DV${f?noteMark(f):""}</dd></div>`}).join("")+"</dl>"}else if(S.tab==="absorption"){const l=sourceOf(e),h=VNUTS.flatMap(r=>{const c=val(e,r.id)===null?null:noteFor(S.sel,r.id);return c&&ABSORB_NOTES.has(c.id)?[{n:r,note:c}]:[]});for(const{note:r}of h)i.add(r);d="<h4>Absorption</h4>"+(h.length?h.map(({n:r,note:c})=>`
        <div class="biorow curated">
          <div class="biohead"><b>${esc(r.label)}</b>
            <span class="biowhen">this food</span></div>
          <p>${esc(c.text)}</p>
        </div>`).join(""):"")+(l.length?l.map(r=>{const c=nut(r),f=affecting(r).map(g=>`
          <div class="biorow ${g.direction}">
            ...
            <p>${esc(g.text)}</p>
          </div>`).join("");return`<h5 class="biofor">${esc(c.label)}</h5>${f}`}).join(""):`<p class="nodatanote">Nothing on record applies to this food. That means no
       ...
       <button class="absorbmore" type="button" data-dlg="bio">Sources</button></p>`}else{const l=NUTS.filter(u=>u.group===S.tab),h=l.filter(u=>!u.evidence),r=h.map(u=>t(u.id)).filter(u=>u!==null),c=Math.max(...r,1e-4),f=lensIds(),g=h.length-r.length;d=`<h4>${esc(GROUPS.find(u=>u.id===S.tab)?.label||S.tab)}</h4>
      <dl>`+l.map(u=>{if(u.evidence){const v=ev(slugAt(S.sel),u.id),w=v&&(v.state==="measured"||v.state==="range"||v.state==="estimated"),T=v?.sources?.length?` <span class="pc">· ${esc(v.sources.map(k=>SRCS[k]?.country??k).join(", "))}</span>`:"";return`<div class="drow${f.has(u.id)?" lensrow":""}" style="display:block">
            <div style="display:flex;justify-content:space-between;gap:10px">
              <dt>${esc(u.label)}</dt>
              <dd>${w?`${esc(evText(v,u.dp))} ${esc(u.unit)}${T}`:`<span class="nodata">${esc(evText(v,u.dp))}</span>`}</dd>
            </div>
          </div>`}const m=t(u.id),b=m===null,p=!b&&u.dv?Math.round(m/u.dv*100):null,y=b?null:noteFor(S.sel,u.id);return y&&i.add(y),`<div class="drow${f.has(u.id)?" lensrow":""}" style="display:block">
          <div style="display:flex;justify-content:space-between;gap:10px">
            <dt>${esc(u.label)}</dt>
            <dd>${b?'<span class="nodata">not measured</span>':`${m.toFixed(u.dp)} ${u.unit}${p!==null?` <span class="pc">· ${p}%</span>`:""}${y?noteMark(y):""}`}</dd>
          </div>
          ${b?"":`<div class="minibar" aria-hidden="true"><i style="width:${(m/c*100).toFixed(1)}%"></i></div>`}
        </div>`}).join("")+"</dl>"+(g?`<p class="nodatanote">${g===l.length?"USDA publishes no figures at all for this group in this food.":`USDA publishes no figure for ${g} of the ${l.length}.`}
  ...
  <form method="dialog">
      ...
      <div class="tabs" role="tablist" aria-label="Nutrient detail sections">
        ${o.map(([l,h,r])=>`
          <button type="button" role="tab" data-tab="${l}" id="tab-${l}"
            ...
            tabindex="${S.tab===l?0:-1}">${r}<span>${h}</span></button>`).join("")}
      </div>
      <div class="dbody" id="tabp" role="tabpanel" aria-labelledby="tab-${S.tab}" tabindex="0">${d}${[...i].map(l=>`<p class="nodatanote"><sup class="fnote">${esc(l.marker)}</sup>
    ...
    <div class="dayfoot">
      ...
      <span class="push">${e.length} food${e.length===1?"":"s"} ·
    ...
    </div>`}const FAO_SCORED=new Set(FAO_PATTERN.flatMap(e=>e.ids)),NON_ESSENTIAL=NUTS.filter(e=>e.group==="amino"&&!FAO_SCORED.has(e.id)).length;function aminoRefsByNutrient(e){const t=new Map;for(const n of dayAminoAcids(e))for(const a of n.ids)t.set(a,{...n,partners:n.ids.filter(s=>s!==a)});return t}function renderDayTotals(e){const t=dayContributors(),n=$("#dayTotals");if(!t.length){n.innerHTML="";return}const a=aminoRefsByNutrient(e),s=new Set,o=GROUPS.filter(i=>S.groups.has(i.id)).map(i=>{const d=e.filter(h=>h.n.group===i.id).map(h=>{const{n:r,total:c,partial:f,from:g,of:u,notes:m}=h,b=a.get(r.id),p=r.dv&&c!==null?c/r.dv*100:b?b.pc:null,y=b&&b.partners.length?`<span class="qual">with ${b.partners.map(v=>nut(v).label.toLowerCase()).join(" and ")}</span>`:"";return m.forEach(v=>s.add(v)),`<div class="totrow${c===null?" none":""}">
        <span class="totname">${esc(r.label)}${m.map(noteMark).join("")}${y}</span>
        <span class="totval">${c===null?'<span class="nodata">not measured</span>':`${c.toFixed(r.dp)} <span class="u">${esc(r.unit)}</span>`}</span>
        ...
        <span class="totpc">${p===null?`<span class="noref" aria-hidden="true">&ndash;</span>
             <span class="sr">no ${r.group==="amino"?"published requirement":"daily value published"}</span>`:`${Math.round(p)}%`}</span>
        <span class="totcov">${f?`from ${g} of ${u}`:""}</span>
      </div>`}).join(""),l=i.id==="amino";return`<div class="totgroup" data-g="${i.id}"><h4>${i.icon}${esc(i.label)}</h4>
      <div class="tothead" aria-hidden="true">
        <span>Nutrient</span><span>Total</span><span></span>
        <span>${l?"of requirement":"of daily value"}</span><span></span>
      </div>${d}
      ${l?`<p class="nodatanote">Against the FAO/WHO adult requirement for
    ...
    <dd>${t.got===null?'<span class="nodata">not measured</span>':`${t.got.toFixed(2)} g <span class="pc">· ${Math.round(t.pc)}%</span>`}</dd></div>`).join("");function weightRow(){const e=(o,i)=>`<button type="button" data-wunit="${o}" aria-pressed="${S.wUnit===o}">${i}</button>`,t=(o,i,d,l,h)=>`<input type="number" inputmode="numeric" id="${o}" data-w value="${i}"
       min="0"${l?` max="${l}"`:""} step="1" aria-label="Body weight in ${h}">
    ...
    <b>Worth pairing</b>
    <ul>${a.map(({x:p,nutrient:y,from:v,via:w})=>`<li class="${p.direction}">
      <span aria-hidden="true">${p.direction==="up"?"↑":"↓"}</span>
      ${esc(w.name)} ${p.direction==="up"?"could help":"could hold back"}
      the ${esc(y.label.toLowerCase())} in ${esc(v.name)}</li>`).join("")}</ul>
    <span class="paircaveat">Only in the same meal. This list is a day, so it cannot know
    whether any of these were eaten together.</span></div>`:"")+DAY_NOTES.map(([p,y])=>`<div><b>${esc(p)}</b> ${esc(y.replace(/\s+/g," "))}</div>`).join("")+"</div>";if(!t.length){n.innerHTML=`<div class="dhead"><h3>Your day</h3>
      <div class="per">nothing added yet</div></div>
      <div class="dbody"><p class="nodatanote" style="margin-top:0">Add a food and its
      totals appear here, in units and as a percentage of a daily value.</p>${o}</div>`;return}const i=totalOf(e,"kcal"),d=totalOf(e,"protein"),l=totalOf(e,"fiber"),h=[i,d,l].map(p=>{const y=p.n.dv&&p.total!==null?Math.round(p.total/p.n.dv*100):null,v=p.partial?` <span class="cov">from ${p.from} of ${p.of}</span>`:"";return`<div class="drow"><dt>${esc(p.n.label)}</dt>
      <dd>${fmtTotal(p.total,p.n)}${y===null?"":` <span class="pc">· ${y}%</span>`}${v}</dd></div>`}).join(""),r=dayProteinQuality(e),f=["ala","la"].every(p=>!totalOf(e,p).partial)?omegaRatio({v:e.map(p=>p.total)}):null,{short:g,over:u,budget:m}=dayStanding(e),b=p=>`<button class="jump" type="button" data-daysort="${esc(p.id)}">
    ...
    <div class="dbody">
      ...
      ${r?`<dl>
        <div class="drow"><dt>Amino acid score</dt>
          <dd class="${r.score>=100?"pc":""}">${r.score}%</dd></div>
        <div class="drow"><dt>${r.score>=100?"Lowest relative to need":"Limiting amino acid"}</dt>
          <dd>${esc(r.limiting)}</dd></div>
        ${r.perKcal!==null?`<div class="drow"><dt>Protein per 100 kcal</dt>
          <dd>${r.perKcal.toFixed(1)} g</dd></div>`:""}
      </dl>
      <p class="nodatanote">${r.score>=100?`Across the whole day this meets the adult FAO/WHO pattern for every essential amino
           ...
           ${esc(r.limiting)} caps it, so adding something richer in that raises the whole day.`}</p>`:`<p class="nodatanote" style="margin-top:0">No score: at least one food in your day has
      ...
      ${f?`<h4 style="margin-top:18px">Omega balance</h4><dl>
        <div class="drow"><dt>Omega-6 : omega-3</dt>
          <dd>${f.flip?`1 : ${f.a.toFixed(1)}`:`${f.a.toFixed(1)} : 1`}</dd></div></dl>`:""}

      ${g.length?`<h4 style="margin-top:18px">Short on</h4>
        <div class="jumps">${g.slice(0,8).map(b).join("")}</div>
      ...
      ${u.length?`<h4 style="margin-top:18px">Comfortable</h4>
        <div class="jumps">${u.slice(0,8).map(b).join("")}</div>`:""}

      ${m.length?`<h4 style="margin-top:18px">Above the guideline</h4>
        <div class="jumps">${m.map(b).join("")}</div>
...
`)],{type:"text/csv;charset=utf-8"}),a=Object.assign(document.createElement("a"),{href:URL.createObjectURL(n),download:t});a.click(),URL.revokeObjectURL(a.href)}function csvTable(){const e=cols(),t=rows(),n=csvQuote,a=basisLabel(),o=[["Food","Also known as","State","Category",...e.map(i=>i.evidence?`${i.label} (${i.unit} per 100 g, evidence)`:`${i.label} (${S.dv&&i.dv?"%DV":i.unit} ${a})`)].map(n).join(",")].concat(t.map(({f:i,i:d})=>[n(i.name),n(i.alt||""),n(i.state),n(i.cat),...e.map(l=>{if(l.evidence)return n(evText(ev(slugAt(d),l.id),l.dp));const h=shown(i,l);return h===null?"":S.dv&&l.dv?Math.round(h/l.dv*100):h})].join(",")));download(o,"vegan-nutrients.csv"),say(`Exported ${t.length} foods and ${e.length} nutrients as CSV.`)}function today(){const e=new Date,t=n=>String(n).padStart(2,"0");return`${e.getFullYear()}-${t(e.getMonth()+1)}-${t(e.getDate())}`}function csvDay(){const e=cols().filter(l=>!l.evidence),t=dayTotals(),n=dayContributors(),a=csvQuote,s=l=>totalOf(t,l),o=a(today()),d=[["Date","Food","State","Grams",...e.map(l=>`${l.label} (${l.unit})`)].map(a).join(",")];for(const{f:l,g:h}of n)d.push([o,a(l.name),a(l.state||""),h,...e.map(r=>{const c=val(l,r.id);return c===null?"":+(c*h/100).toFixed(6)})].join(","));d.push([o,a("Total"),a(""),dayGrams(),...e.map(l=>{const h=s(l.id).total;return h===null?"":+h.toFixed(6)})].join(",")),d.push([o,a("% of daily value"),a(""),"",...e.map(l=>{const h=s(l.id).total;return l.dv&&h!==null?Math.round(h/l.dv*100):""})].join(",")),d.push([o,a("Foods measured"),a(""),"",...e.map(l=>`${s(l.id).from} of ${s(l.id).of}`).map(a)].join(",")),download(d,"my-day.csv"),say(`Exported your day, ${n.length} foods and ${e.length} nutrients, as CSV.`)}const csv=()=>S.view==="day"?csvDay():csvTable();$("#csvBtn").onclick=csv;function renderNutPick(e=new Set){$("#nutPick").innerHTML=GROUPS.map(t=>{const n=NUTS.filter(a=>a.group===t.id);return`<div class="lensgroup"><h5>${esc(t.label)}</h5><div class="nutgrid">`+n.map(a=>`<label><input type="checkbox" name="nut" value="${esc(a.id)}"
    ...
    ${[...e.entries()].sort((a,s)=>t(a[0])-t(s[0])).map(([a,s])=>{const o=a.split("+").map(d=>nutOpt(d)?.label??d).join(", ").replace(/, ([^,]*)$/," and $1"),i=s.map(d=>`
        <div class="biorow ${d.direction}">
          ...
          <p>${esc(d.text)}</p>
          ${d.cites.map(l=>`<cite>${esc(X.sources[l]??l)}</cite>`).join("")}
