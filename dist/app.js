"use strict";const NUTS=DATA.nutrients,FOODS=DATA.foods,GROUPS=[{id:"macro",label:"Macronutrients",icon:I.macro},{id:"fats",label:"Omega & fats",icon:I.fats},{id:"amino",label:"Amino acids",icon:I.amino},{id:"vitamin",label:"Vitamins",icon:I.vit},{id:"mineral",label:"Minerals",icon:I.min},{id:"carbdetail",label:"Carbohydrate detail",icon:I.carb},{id:"acids",label:"Organic acids",icon:I.acid},{id:"plant",label:"Plant compounds",icon:I.plant},{id:"other",label:"Anti-nutrients",icon:I.other}],VNUTS=NUTS.filter(e=>!e.evidence),IDX=new Map(VNUTS.map((e,t)=>[e.id,t])),CATS=[...new Set(FOODS.map(e=>e.cat))].sort(),slugify=e=>`${e.name} ${e.state||""}`.toLowerCase().trim().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,""),SLUGS=FOODS.map(slugify),BY_SLUG=new Map(SLUGS.map((e,t)=>[e,t])),BY_ID=new Map(NUTS.map(e=>[e.id,e])),nutOpt=e=>BY_ID.get(e),nut=e=>{const t=nutOpt(e);if(!t)throw new Error(`unknown nutrient id: ${e}`);return t},foodAt=e=>{const t=FOODS[e];if(!t)throw new Error(`no food at index ${e}`);return t},slugAt=e=>{const t=SLUGS[e];if(t===void 0)throw new Error(`no food at index ${e}`);return t},foodBySlug=e=>{const t=BY_SLUG.get(e);return t===void 0?void 0:FOODS[t]},groupOf=e=>{const t=GROUPS.find(n=>n.id===e);if(!t)throw new Error(`unknown nutrient group: ${e}`);return t},isGroup=e=>GROUPS.some(t=>t.id===e),RENAMED={"navy-beans-cooked":"haricot-beans-cooked"},currentSlug=e=>RENAMED[e]||e,NOTES=DATA.notes||[],NOTE_AT=new Map;for(const e of NOTES)for(const[t,n]of Object.entries(e.cells||{}))for(const a of n)NOTE_AT.set(`${t} ${a}`,e);const noteFor=(e,t)=>NOTE_AT.get(`${slugAt(e)} ${t}`)||null,ABSORB_NOTES=new Set(["oxalate-high","oxalate-low"]),noteMark=e=>`<sup class="fnote" aria-hidden="true">${esc(e.marker)}</sup><span class="sr">, ${esc(e.short)}</span>`,INTERACTIONS=X.interactions,AFFECTING=new Map;for(const e of INTERACTIONS)for(const t of e.affects)AFFECTING.set(t,[...AFFECTING.get(t)||[],e]);const ACTING=new Map;for(const e of INTERACTIONS)e.agent.kind==="nutrient"&&ACTING.set(e.agent.id,[...ACTING.get(e.agent.id)||[],e]);const affecting=e=>AFFECTING.get(e)||[],acting=e=>ACTING.get(e)||[],hasInteractions=e=>affecting(e).length>0||acting(e).length>0,affectedLabels=e=>e.affects.map(t=>nutOpt(t)?.label??t),RANK_DEPTH=10;function sourceOf(e){const t=[];for(const n of VNUTS){if(!hasInteractions(n.id))continue;const a=val(e,n.id);if(!(a===null||a<=0))if(n.dv)a/n.dv>=.1&&t.push(n.id);else{const s=IDX.get(n.id);if(s===void 0)continue;let o=0;for(const i of FOODS){const c=i.v[s];if(typeof c=="number"&&c>a&&++o>=RANK_DEPTH)break}o<RANK_DEPTH&&t.push(n.id)}}return t}function gapEvidence(e){const t=nutOpt(e);if(!t||t.evidence)return null;let n=0,a=0,s=0,o=0,i=null,c=null;return FOODS.forEach((r,p)=>{const l=val(r,e);if(l===null){s++;return}if(l===0){a++;return}if(n++,noteFor(p,e)?.id==="fortified"){o++;return}if(t.dv){const u=l/t.dv*100;(i===null||u>i)&&(i=u,c=r.name)}}),{above:n,zero:a,unassayed:s,of:FOODS.length,fortified:o,bestPc:i,bestFood:c}}function gapEvidenceText(e){const t=e.nutrients.map(n=>{const a=gapEvidence(n);if(!a)return"";const s=nutOpt(n)?.label??n,o=[`${a.above} of ${a.of} foods ${a.above===1?"has":"have"} any at all`];return a.fortified&&o.push(`${a.fortified} of those ${a.fortified===1?"is":"are"} fortification rather than the food`),a.zero&&o.push(`${a.zero} were measured and found to contain none`),a.unassayed&&o.push(`${a.unassayed} ${a.unassayed===1?"was":"were"} never assayed`),a.bestPc!==null&&a.bestFood&&o.push(`the best unfortified is ${esc(a.bestFood)}, at ${Math.round(a.bestPc)}% of a daily value per 100 g`),`<li><b>${esc(s)}:</b> ${o.join("; ")}.</li>`}).filter(Boolean);return t.length?`<ul class="gapev"><li class="gapevhead">Measured from this table,
    ...
    <tr>
      ...
      ${o}
    </tr><tr>${i}</tr>`;const c=new Set,r=new Set;$("#tbody").innerHTML=n.length?n.map(({f:d,i:h})=>{const f=S.basis==="kcal"?gramsPer100kcal(d):null,b=seasoningPortion(d,slugAt(h));return`
    <tr data-i="${h}" ${S.sel===h?'aria-selected="true"':""}>
      <td class="food${a?" sorted":""}"><div class="fcell">
        <span class="sw" style="--c:${d.colour}" aria-hidden="true"></span>
        <button class="fname" type="button" data-pick="${h}" data-name="${esc(d.name)}">
        ...
        <button class="fav" type="button" data-fav="${h}" aria-pressed="${isFav(h)}">
          ${isFav(h)?I.heartFull:I.heart}
          <span class="sr">${isFav(h)?"Remove":"Add"} ${esc(d.name)} ${isFav(h)?"from":"to"} favourites</span>
        </button></div></td>
      ${t.map(m=>{if(m.evidence){if(S.dv)return`<td class="num ${colClass(m)}" data-g="${m.group}" data-n="${esc(m.id)}" data-ev="nodv"><span class="noref" aria-hidden="true">&ndash;</span><span class="sr">no daily value published</span></td>`;const w=ev(slugAt(h),m.id),k=evGrade(slugAt(h),w)==="proxy"?' data-match="proxy"':"";return w?.state==="estimated"&&r.add("calc"),k&&r.add("proxy"),`<td class="num ${colClass(m)}" data-g="${m.group}" data-n="${esc(m.id)}" data-ev="${w?w.state:"none"}"${k}>${esc(evText(w,m.dp))}</td>`}const g=shown(d,m),v=g===0||g===null,y=g===null?null:noteFor(h,m.id);return y&&c.add(y),`<td class="num${v?" low":""} ${colClass(m)}" data-g="${m.group}" data-n="${esc(m.id)}">${fmt(g,m)}${y?noteMark(y):""}</td>`}).join("")}
    </tr>`}).join(""):`<tr><td class="empty" colspan="${t.length+1}">${emptyState()}</td></tr>`;const p=lensById(S.lens);$("#cap").textContent=(n.length===FOODS.length?`${FOODS.length} vegan foods`:`Showing ${n.length} of ${FOODS.length} vegan foods`)+(t.length===NUTS.length?`, all ${t.length} nutrient columns`:`, ${t.length} of ${NUTS.length} nutrient columns`)+`. Values ${basisLabel()} of food`+(S.dv?", shown as % of adult daily value.":".")+(S.favsOnly?" Favourites only.":"")+(p?` ${p.name} preset applied.`:"")+(S.dv&&S.basis==="kcal"?" 5% here is a full day's worth at 2000 kcal.":"");const l={calc:["calc","Calculated by the source","The database published this as a calculation or an inference from a similar food rather than as an assay of this one. Japan's tables print such figures in parentheses."],proxy:["~","Measured in a proxy food","The figure itself was measured. What is only approximate is the food: the closest row in that database was a related species or a notably different preparation, so it stands in for the one named here."]},u=$("#noteKey");u.hidden=!c.size&&!r.size,u.innerHTML=[...r].map(d=>{const[h,f,b]=l[d];return`<span><sup class="fnote">${esc(h)}</sup> <b>${esc(f)}.</b> ${esc(b)}</span>`}).concat([...c].map(d=>`<span><sup class="fnote">${esc(d.marker)}</sup> <b>${esc(d.short)}.</b>
     ${esc(d.text)}</span>`)).join(""),syncHeadOffset()}function syncHeadOffset(){const e=$("#thead").rows[0];if(!e)return;const t=e.getBoundingClientRect().height;t&&$("#grid").style.setProperty("--head1",`${Math.floor(t)}px`);const n=e.cells[0],a=n&&n.getBoundingClientRect().width;a&&$("#grid").style.setProperty("--foodw",`${a}px`);const s=$("#scroller").clientWidth;s&&$("#grid").style.setProperty("--scrollw",`${s}px`)}addEventListener("resize",syncHeadOffset);function emptyState(){return S.favsOnly&&!S.favs.size?`<b>No favourites yet</b>Star a food with the heart button to build a shortlist,
      ...
      <dd>${n.flip?`1 : ${n.a.toFixed(1)}`:`${n.a.toFixed(1)} : 1`}</dd></div>`);const o=t?t.score>=100?"Meets the adult FAO/WHO pattern for every essential amino acid.":`Scored against the FAO/WHO adult pattern. ${esc(t.limiting)} caps the
           score; pairing this food with one richer in it raises the total.`:"";return`<h4 style="margin-top:18px">Protein quality</h4><dl>${s}</dl>`+(o?`<p style="font-size:11.5px;color:var(--faint);margin:8px 0 0;line-height:1.4">${o}</p>`:"")}function renderDetail(){const e=foodAt(S.sel),t=r=>shown(e,nut(r)),n=r=>evText(ev(slugAt(S.sel),r),nut(r).dp),a=S.day.find(r=>r.slug===slugAt(S.sel)),s=["fats","vitamin","mineral","carbdetail","acids","amino","plant","other"],o=[["overview","Overview",I.macro],...s.map(r=>groupOf(r)).map(r=>[r.id,r.label,r.icon]),["absorption","Absorption",I.eye]],i=new Set;let c;if(S.tab==="overview"){const r=["kcal","protein","carbs","fiber","solfibre","insolfibre","resstarch","fat","satfat"],p=NUTS.filter(l=>l.dv!==null&&l.dv>0&&l.group!=="macro").flatMap(l=>{const u=t(l.id);return u===null?[]:[{n:l,pc:u/l.dv*100}]}).filter(l=>l.pc>0).sort((l,u)=>u.pc-l.pc).slice(0,6);c="<h4>Macronutrients</h4><dl>"+r.map(l=>{const u=nut(l),d=l==="fiber"||l==="satfat"||!!u.evidence;if(u.evidence){const b=ev(slugAt(S.sel),l),m=b&&(b.state==="measured"||b.state==="range"||b.state==="estimated");return`<div class="drow sub"><dt>${esc(u.label)}</dt>
          <dd>${m?`${esc(n(l))} ${esc(u.unit)}`:`<span class="nodata">${esc(n(l))}</span>`}</dd></div>`}const h=t(l),f=l==="kcal"&&h!==null?` <span class="pc">· ${Math.round(h*4.184)} kJ</span>`:"";return`<div class="drow${d?" sub":""}"><dt>${esc(u.label)}</dt>
        <dd>${h===null?'<span class="nodata">not measured</span>':`${h.toFixed(u.dp)} ${u.unit}${f}`}</dd></div>`}).join("")+"</dl>"+proteinQualityBlock(e)+'<h4 style="margin-top:18px">Top nutrients</h4><dl>'+p.map(({n:l,pc:u})=>{const d=noteFor(S.sel,l.id);return d&&i.add(d),`<div class="drow"><dt>${esc(l.label)}</dt>
          <dd class="pc">${Math.round(u)}% DV${d?noteMark(d):""}</dd></div>`}).join("")+"</dl>"}else if(S.tab==="absorption"){const r=sourceOf(e),p=VNUTS.flatMap(l=>{const u=val(e,l.id)===null?null:noteFor(S.sel,l.id);return u&&ABSORB_NOTES.has(u.id)?[{n:l,note:u}]:[]});for(const{note:l}of p)i.add(l);c="<h4>Absorption</h4>"+(p.length?p.map(({n:l,note:u})=>`
        <div class="biorow curated">
          <div class="biohead"><b>${esc(l.label)}</b>
            <span class="biowhen">this food</span></div>
          <p>${esc(u.text)}</p>
        </div>`).join(""):"")+(r.length?r.map(l=>{const u=nut(l),d=affecting(l).map(h=>`
          <div class="biorow ${h.direction}">
            ...
            <p>${esc(h.text)}</p>
          </div>`).join("");return`<h5 class="biofor">${esc(u.label)}</h5>${d}`}).join(""):`<p class="nodatanote">Nothing on record applies to this food. That means no
       ...
       <button class="absorbmore" type="button" data-dlg="bio">Sources</button></p>`}else{const r=NUTS.filter(f=>f.group===S.tab),p=r.filter(f=>!f.evidence),l=p.map(f=>t(f.id)).filter(f=>f!==null),u=Math.max(...l,1e-4),d=lensIds(),h=p.length-l.length;c=`<h4>${esc(GROUPS.find(f=>f.id===S.tab)?.label||S.tab)}</h4>
      <dl>`+r.map(f=>{if(f.evidence){const y=ev(slugAt(S.sel),f.id),w=y&&(y.state==="measured"||y.state==="range"||y.state==="estimated"),k=y?.sources?.length?` <span class="pc">· ${esc(y.sources.map(T=>SRCS[T]?.short??T).join(", "))}</span>`:"";return`<div class="drow${d.has(f.id)?" lensrow":""}" style="display:block">
            <div style="display:flex;justify-content:space-between;gap:10px">
              <dt>${esc(f.label)}</dt>
              <dd>${w?`${esc(evText(y,f.dp))} ${esc(f.unit)}${k}`:`<span class="nodata">${esc(evText(y,f.dp))}</span>`}</dd>
            </div>
          </div>`}const b=t(f.id),m=b===null,g=!m&&f.dv?Math.round(b/f.dv*100):null,v=m?null:noteFor(S.sel,f.id);return v&&i.add(v),`<div class="drow${d.has(f.id)?" lensrow":""}" style="display:block">
          <div style="display:flex;justify-content:space-between;gap:10px">
            <dt>${esc(f.label)}</dt>
            <dd>${m?'<span class="nodata">not measured</span>':`${b.toFixed(f.dp)} ${f.unit}${g!==null?` <span class="pc">· ${g}%</span>`:""}${v?noteMark(v):""}`}</dd>
          </div>
          ${m?"":`<div class="minibar" aria-hidden="true"><i style="width:${(b/u*100).toFixed(1)}%"></i></div>`}
        </div>`}).join("")+"</dl>"+(h?`<p class="nodatanote">${h===r.length?"USDA publishes no figures at all for this group in this food.":`USDA publishes no figure for ${h} of the ${r.length}.`}
  ...
  <form method="dialog">
      ...
      <div class="tabs" role="tablist" aria-label="Nutrient detail sections">
        ${o.map(([r,p,l])=>`
          <button type="button" role="tab" data-tab="${r}" id="tab-${r}"
            ...
            tabindex="${S.tab===r?0:-1}">${l}<span>${p}</span></button>`).join("")}
      </div>
    ...
    <div class="dayfoot">
      ...
      <span class="push">${e.length} food${e.length===1?"":"s"} ·
        ...
        <span class="totpc">${g===null?`<span class="noref" aria-hidden="true">&ndash;</span>
             <span class="sr">no ${l.group==="amino"?"published requirement":"daily value published"}</span>`:`${Math.round(g)}%`}</span>
        <span class="totcov">${d?`from ${h} of ${f}`:""}</span>
    ...
    whether any of these were eaten together.</span></div>`:"")+DAY_NOTES.map(([g,v])=>`<div><b>${esc(g)}</b> ${esc(v.replace(/\s+/g," "))}</div>`).join("")+"</div>";if(!t.length){n.innerHTML=`<div class="dhead"><h3>Your day</h3>
      ...
      <dd>${fmtTotal(g.total,g.n)}${v===null?"":` <span class="pc">· ${v}%</span>`}${y}</dd></div>`}).join(""),l=dayProteinQuality(e),d=["ala","la"].every(g=>!totalOf(e,g).partial)?omegaRatio({v:e.map(g=>g.total)}):null,{short:h,over:f,budget:b}=dayStanding(e),m=g=>`<button class="jump" type="button" data-daysort="${esc(g.id)}">
    ...
    <div class="dbody">
      ...
      ${h.length?`<h4 style="margin-top:18px">Short on</h4>
