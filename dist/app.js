"use strict";const NUTS=DATA.nutrients,FOODS=DATA.foods,GROUPS=[{id:"macro",label:"Macronutrients",icon:I.macro},{id:"fats",label:"Omega & fats",icon:I.fats},{id:"amino",label:"Amino acids",icon:I.amino},{id:"vitamin",label:"Vitamins",icon:I.vit},{id:"mineral",label:"Minerals",icon:I.min},{id:"carbdetail",label:"Carbohydrate detail",icon:I.carb},{id:"acids",label:"Organic acids",icon:I.acid},{id:"plant",label:"Plant compounds",icon:I.plant},{id:"other",label:"Anti-nutrients",icon:I.other}],VNUTS=NUTS.filter(e=>!e.evidence),IDX=new Map(VNUTS.map((e,t)=>[e.id,t])),CATS=[...new Set(FOODS.map(e=>e.cat))].sort(),slugify=e=>`${e.name} ${e.state||""}`.toLowerCase().trim().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,""),SLUGS=FOODS.map(slugify),BY_SLUG=new Map(SLUGS.map((e,t)=>[e,t])),BY_ID=new Map(NUTS.map(e=>[e.id,e])),nutOpt=e=>BY_ID.get(e),nut=e=>{const t=nutOpt(e);if(!t)throw new Error(`unknown nutrient id: ${e}`);return t},foodAt=e=>{const t=FOODS[e];if(!t)throw new Error(`no food at index ${e}`);return t},slugAt=e=>{const t=SLUGS[e];if(t===void 0)throw new Error(`no food at index ${e}`);return t},foodBySlug=e=>{const t=BY_SLUG.get(e);return t===void 0?void 0:FOODS[t]},groupOf=e=>{const t=GROUPS.find(n=>n.id===e);if(!t)throw new Error(`unknown nutrient group: ${e}`);return t},isGroup=e=>GROUPS.some(t=>t.id===e),RENAMED={"navy-beans-cooked":"haricot-beans-cooked"},currentSlug=e=>RENAMED[e]||e,NOTES=DATA.notes||[],NOTE_AT=new Map;for(const e of NOTES)for(const[t,n]of Object.entries(e.cells||{}))for(const a of n)NOTE_AT.set(`${t} ${a}`,e);const noteFor=(e,t)=>NOTE_AT.get(`${slugAt(e)} ${t}`)||null,ABSORB_NOTES=new Set(["oxalate-high","oxalate-low"]),noteMark=e=>`<sup class="fnote" aria-hidden="true">${esc(e.marker)}</sup><span class="sr">, ${esc(e.short)}</span>`,INTERACTIONS=X.interactions,AFFECTING=new Map;for(const e of INTERACTIONS)for(const t of e.affects)AFFECTING.set(t,[...AFFECTING.get(t)||[],e]);const PREPARING=new Map;for(const e of PREP.preparations)for(const t of e.applies)PREPARING.set(t,[...PREPARING.get(t)||[],e]);const preparing=e=>PREPARING.get(e)||[],prepMeasured=(e,t)=>e.measuredIn.includes(t),prepComponent=e=>e.component?nutOpt(e.component)?.label??e.component:e.componentLabel??"",prepHasColumn=e=>!!e.component,ACTING=new Map;for(const e of INTERACTIONS)e.agent.kind==="nutrient"&&ACTING.set(e.agent.id,[...ACTING.get(e.agent.id)||[],e]);const affecting=e=>AFFECTING.get(e)||[],acting=e=>ACTING.get(e)||[],hasInteractions=e=>affecting(e).length>0||acting(e).length>0,affectedLabels=e=>e.affects.map(t=>nutOpt(t)?.label??t),RANK_DEPTH=10;function sourceOf(e){const t=[];for(const n of VNUTS){if(!hasInteractions(n.id))continue;const a=val(e,n.id);if(!(a===null||a<=0))if(n.dv)a/n.dv>=.1&&t.push(n.id);else{const s=IDX.get(n.id);if(s===void 0)continue;let o=0;for(const i of FOODS){const c=i.v[s];if(typeof c=="number"&&c>a&&++o>=RANK_DEPTH)break}o<RANK_DEPTH&&t.push(n.id)}}return t}function gapEvidence(e){const t=nutOpt(e);if(!t||t.evidence)return null;let n=0,a=0,s=0,o=0,i=null,c=null;return FOODS.forEach((r,p)=>{const d=val(r,e);if(d===null){s++;return}if(d===0){a++;return}if(n++,noteFor(p,e)?.id==="fortified"){o++;return}if(t.dv){const u=d/t.dv*100;(i===null||u>i)&&(i=u,c=r.name)}}),{above:n,zero:a,unassayed:s,of:FOODS.length,fortified:o,bestPc:i,bestFood:c}}function gapEvidenceText(e){const t=e.nutrients.map(n=>{const a=gapEvidence(n);if(!a)return"";const s=nutOpt(n)?.label??n,o=[`${a.above} of ${a.of} foods ${a.above===1?"has":"have"} any at all`];return a.fortified&&o.push(`${a.fortified} of those ${a.fortified===1?"is":"are"} fortification rather than the food`),a.zero&&o.push(`${a.zero} were measured and found to contain none`),a.unassayed&&o.push(`${a.unassayed} ${a.unassayed===1?"was":"were"} never assayed`),a.bestPc!==null&&a.bestFood&&o.push(`the best unfortified is ${esc(a.bestFood)}, at ${Math.round(a.bestPc)}% of a daily value per 100 g`),`<li><b>${esc(s)}:</b> ${o.join("; ")}.</li>`}).filter(Boolean);return t.length?`<ul class="gapev"><li class="gapevhead">Measured from this table,
    ...
    <tr>
      ...
      ${o}
    </tr><tr>${i}</tr>`;const c=new Set,r=new Set;$("#tbody").innerHTML=n.length?n.map(({f:l,i:f})=>{const h=S.basis==="kcal"?gramsPer100kcal(l):null,b=seasoningPortion(l,slugAt(f));return`
    <tr data-i="${f}" ${S.sel===f?'aria-selected="true"':""}>
     ...
     ${esc(l.text)}</span>`)).join(""),syncHeadOffset()}function syncHeadOffset(){const e=$("#thead").rows[0];if(!e)return;const t=e.getBoundingClientRect().height;t&&$("#grid").style.setProperty("--head1",`${Math.floor(t)}px`);const n=e.cells[0],a=n&&n.getBoundingClientRect().width;a&&$("#grid").style.setProperty("--foodw",`${a}px`);const s=$("#scroller").clientWidth;s&&$("#grid").style.setProperty("--scrollw",`${s}px`)}addEventListener("resize",syncHeadOffset);function emptyState(){return S.favsOnly&&!S.favs.size?`<b>No favourites yet</b>Star a food with the heart button to build a shortlist,
      ...
      <dd>${n.flip?`1 : ${n.a.toFixed(1)}`:`${n.a.toFixed(1)} : 1`}</dd></div>`);const o=t?t.score>=100?"Meets the adult FAO/WHO pattern for every essential amino acid.":`Scored against the FAO/WHO adult pattern. ${esc(t.limiting)} caps the
           score; pairing this food with one richer in it raises the total.`:"";return`<h4 style="margin-top:18px">Protein quality</h4><dl>${s}</dl>`+(o?`<p style="font-size:11.5px;color:var(--faint);margin:8px 0 0;line-height:1.4">${o}</p>`:"")}function renderDetail(){const e=foodAt(S.sel),t=r=>shown(e,nut(r)),n=r=>evText(ev(slugAt(S.sel),r),nut(r).dp),a=S.day.find(r=>r.slug===slugAt(S.sel)),s=["fats","vitamin","mineral","carbdetail","acids","amino","plant","other"],o=[["overview","Overview",I.macro],...s.map(r=>groupOf(r)).map(r=>[r.id,r.label,r.icon]),["absorption","Absorption",I.eye]],i=new Set;let c;if(S.tab==="overview"){const r=["kcal","protein","carbs","fiber","solfibre","insolfibre","resstarch","fat","satfat"],p=NUTS.filter(d=>d.dv!==null&&d.dv>0&&d.group!=="macro").flatMap(d=>{const u=t(d.id);return u===null?[]:[{n:d,pc:u/d.dv*100}]}).filter(d=>d.pc>0).sort((d,u)=>u.pc-d.pc).slice(0,6);c="<h4>Macronutrients</h4><dl>"+r.map(d=>{const u=nut(d),l=d==="fiber"||d==="satfat"||!!u.evidence;if(u.evidence){const b=ev(slugAt(S.sel),d),m=b&&(b.state==="measured"||b.state==="range"||b.state==="estimated");return`<div class="drow sub"><dt>${esc(u.label)}</dt>
          <dd>${m?`${esc(n(d))} ${esc(u.unit)}`:`<span class="nodata">${esc(n(d))}</span>`}</dd></div>`}const f=t(d),h=d==="kcal"&&f!==null?` <span class="pc">· ${Math.round(f*4.184)} kJ</span>`:"";return`<div class="drow${l?" sub":""}"><dt>${esc(u.label)}</dt>
        <dd>${f===null?'<span class="nodata">not measured</span>':`${f.toFixed(u.dp)} ${u.unit}${h}`}</dd></div>`}).join("")+"</dl>"+proteinQualityBlock(e)+'<h4 style="margin-top:18px">Top nutrients</h4><dl>'+p.map(({n:d,pc:u})=>{const l=noteFor(S.sel,d.id);return l&&i.add(l),`<div class="drow"><dt>${esc(d.label)}</dt>
          <dd class="pc">${Math.round(u)}% DV${l?noteMark(l):""}</dd></div>`}).join("")+"</dl>"}else if(S.tab==="absorption"){const r=sourceOf(e),p=VNUTS.flatMap(u=>{const l=val(e,u.id)===null?null:noteFor(S.sel,u.id);return l&&ABSORB_NOTES.has(l.id)?[{n:u,note:l}]:[]});for(const{note:u}of p)i.add(u);const d=preparing(slugAt(S.sel));c=(d.length?"<h4>Preparation</h4>"+d.map(u=>{const l=prepMeasured(u,slugAt(S.sel));return`
        <div class="biorow ${u.direction}${l?"":" carried"}">
          <div class="biohead">
            <span class="bioarrow" aria-hidden="true">${u.direction==="up"?"↑":"↓"}</span>
            <b>${esc(agentLabel(u.agent))}</b>
            <span class="biowhen">${esc(prepComponent(u))}</span>
          </div>
          <p>${esc(u.text)}</p>
       ...
       <button class="absorbmore" type="button" data-dlg="bio">Sources</button></p>`}else{const r=NUTS.filter(h=>h.group===S.tab),p=r.filter(h=>!h.evidence),d=p.map(h=>t(h.id)).filter(h=>h!==null),u=Math.max(...d,1e-4),l=lensIds(),f=p.length-d.length;c=`<h4>${esc(GROUPS.find(h=>h.id===S.tab)?.label||S.tab)}</h4>
  ...
  <form method="dialog">
      ...
      <div class="tabs" role="tablist" aria-label="Nutrient detail sections">
        ${o.map(([r,p,d])=>`
          <button type="button" role="tab" data-tab="${r}" id="tab-${r}"
            ...
            tabindex="${S.tab===r?0:-1}">${d}<span>${p}</span></button>`).join("")}
      </div>
    ...
    <div class="dayfoot">
      ...
      <span class="push">${e.length} food${e.length===1?"":"s"} ·
        ...
        <span class="totpc">${g===null?`<span class="noref" aria-hidden="true">&ndash;</span>
             <span class="sr">no ${d.group==="amino"?"published requirement":"daily value published"}</span>`:`${Math.round(g)}%`}</span>
    ...
    <dd>${t.got===null?'<span class="nodata">not measured</span>':`${t.got.toFixed(2)} g <span class="pc">· ${Math.round(t.pc)}%</span>`}</dd></div>`).join("");function weightRow(){const e=(o,i)=>`<button type="button" data-wunit="${o}" aria-pressed="${S.wUnit===o}">${i}</button>`,t=(o,i,c,r,p)=>`<input type="number" inputmode="numeric" id="${o}" data-w value="${i}"
    ...
    whether any of these were eaten together.</span></div>`:"")+DAY_NOTES.map(([g,v])=>`<div><b>${esc(g)}</b> ${esc(v.replace(/\s+/g," "))}</div>`).join("")+"</div>";if(!t.length){n.innerHTML=`<div class="dhead"><h3>Your day</h3>
      ...
      totals appear here, in units and as a percentage of a daily value.</p>${o}</div>`;return}const i=totalOf(e,"kcal"),c=totalOf(e,"protein"),r=totalOf(e,"fiber"),p=[i,c,r].map(g=>{const v=g.n.dv&&g.total!==null?Math.round(g.total/g.n.dv*100):null,y=g.partial?` <span class="cov">from ${g.from} of ${g.of}</span>`:"";return`<div class="drow"><dt>${esc(g.n.label)}</dt>
      <dd>${fmtTotal(g.total,g.n)}${v===null?"":` <span class="pc">· ${v}%</span>`}${y}</dd></div>`}).join(""),d=dayProteinQuality(e),l=["ala","la"].every(g=>!totalOf(e,g).partial)?omegaRatio({v:e.map(g=>g.total)}):null,{short:f,over:h,budget:b}=dayStanding(e),m=g=>`<button class="jump" type="button" data-daysort="${esc(g.id)}">
...
`)],{type:"text/csv;charset=utf-8"}),a=Object.assign(document.createElement("a"),{href:URL.createObjectURL(n),download:t});a.click(),URL.revokeObjectURL(a.href)}function csvTable(){const e=cols(),t=rows(),n=csvQuote,a=basisLabel(),o=[["Food","Also known as","State","Category",...e.map(i=>i.evidence?`${i.label} (${i.unit} per 100 g, evidence)`:`${i.label} (${S.dv&&i.dv?"%DV":i.unit} ${a})`)].map(n).join(",")].concat(t.map(({f:i,i:c})=>[n(i.name),n(i.alt||""),n(i.state),n(i.cat),...e.map(r=>{if(r.evidence)return n(evText(ev(slugAt(c),r.id),r.dp));const p=shown(i,r);return p===null?"":S.dv&&r.dv?Math.round(p/r.dv*100):p})].join(",")));download(o,"vegan-nutrients.csv"),say(`Exported ${t.length} foods and ${e.length} nutrients as CSV.`)}function today(){const e=new Date,t=n=>String(n).padStart(2,"0");return`${e.getFullYear()}-${t(e.getMonth()+1)}-${t(e.getDate())}`}function csvDay(){const e=cols().filter(r=>!r.evidence),t=dayTotals(),n=dayContributors(),a=csvQuote,s=r=>totalOf(t,r),o=a(today()),c=[["Date","Food","State","Grams",...e.map(r=>`${r.label} (${r.unit})`)].map(a).join(",")];for(const{f:r,g:p}of n)c.push([o,a(r.name),a(r.state||""),p,...e.map(d=>{const u=val(r,d.id);return u===null?"":+(u*p/100).toFixed(6)})].join(","));c.push([o,a("Total"),a(""),dayGrams(),...e.map(r=>{const p=s(r.id).total;return p===null?"":+p.toFixed(6)})].join(",")),c.push([o,a("% of daily value"),a(""),"",...e.map(r=>{const p=s(r.id).total;return r.dv&&p!==null?Math.round(p/r.dv*100):""})].join(",")),c.push([o,a("Foods measured"),a(""),"",...e.map(r=>`${s(r.id).from} of ${s(r.id).of}`).map(a)].join(",")),download(c,"my-day.csv"),say(`Exported your day, ${n.length} foods and ${e.length} nutrients, as CSV.`)}const csv=()=>S.view==="day"?csvDay():csvTable();$("#csvBtn").onclick=csv;function renderNutPick(e=new Set){$("#nutPick").innerHTML=GROUPS.map(t=>{const n=NUTS.filter(a=>a.group===t.id);return`<div class="lensgroup"><h5>${esc(t.label)}</h5><div class="nutgrid">`+n.map(a=>`<label><input type="checkbox" name="nut" value="${esc(a.id)}"
    ...
    foods.</p>`}function prepDialog(){const e=new Map;for(const n of PREP.preparations){const a=n.component??`label:${n.componentLabel}`;e.set(a,[...e.get(a)||[],n])}return`
    ...
    ${[...e.entries()].sort((n,a)=>(IDX.get(n[0])??999)-(IDX.get(a[0])??999)).map(([n,a])=>{const s=a.map(r=>{const p=r.applies.flatMap(d=>{const u=foodBySlug(d);return u?[fullName(u)]:[]});return`
        <div class="biorow ${r.direction}">
          <div class="biohead">
            <span class="bioarrow" aria-hidden="true">${r.direction==="up"?"↑":"↓"}</span>
            <b>${esc(agentLabel(r.agent))}</b>
            <span class="biowhen">${r.applies.length} ${r.applies.length===1?"food":"foods"}</span>
          </div>
          <p>${esc(r.text)}</p>
          <p class="carriednote">Shown on ${esc(andList(p))}.
            ${r.measuredIn.length<r.applies.length?`Measured in ${esc(andList(r.measuredIn.flatMap(d=>{const u=foodBySlug(d);return u?[fullName(u)]:[]})))}; carried over to the rest.`:"Measured in every food it is shown on."}</p>
          ${r.cites.map(d=>`<cite>${esc(PREP.sources[d]??d)}</cite>`).join("")}
        </div>`}).join(""),o=a[0],i=o?prepComponent(o):n,c=o&&!prepHasColumn(o)?`<p class="carriednote">There is no column for this in the table. It is
           made when you damage the food, not held in it, so there is nothing to
           measure per 100 g.</p>`:"";return`<h4>${esc(i)}</h4>${c}${s}`}).join("")}
