"use strict";const NUTS=DATA.nutrients,FOODS=DATA.foods,GROUPS=[{id:"macro",label:"Macronutrients",icon:I.macro},{id:"fats",label:"Omega & fats",icon:I.fats},{id:"amino",label:"Amino acids",icon:I.amino},{id:"vitamin",label:"Vitamins",icon:I.vit},{id:"mineral",label:"Minerals",icon:I.min},{id:"plant",label:"Plant compounds",icon:I.plant}],IDX=new Map(NUTS.map((e,t)=>[e.id,t])),CATS=[...new Set(FOODS.map(e=>e.cat))].sort(),slugify=e=>`${e.name} ${e.state||""}`.toLowerCase().trim().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,""),SLUGS=FOODS.map(slugify),BY_SLUG=new Map(SLUGS.map((e,t)=>[e,t])),RENAMED={"navy-beans-cooked":"haricot-beans-cooked"},currentSlug=e=>RENAMED[e]||e,NOTES=DATA.notes||[],NOTE_AT=new Map;for(const e of NOTES)for(const[t,a]of Object.entries(e.cells||{}))for(const n of a)NOTE_AT.set(`${t} ${n}`,e);const noteFor=(e,t)=>NOTE_AT.get(`${SLUGS[e]} ${t}`)||null,noteMark=e=>`<sup class="fnote" aria-hidden="true">${esc(e.marker)}</sup><span class="sr">, ${esc(e.short)}</span>`,BUILTIN_LENSES=[{id:"eaa",name:"Essential amino acids",ids:["his","ile","leu","lys","met","phe","thr","trp","val"],why:"The nine amino acids the body cannot make and must get from food. A protein is only as useful as its scarcest one, so the lowest of these caps the rest."},{id:"creatine",name:"Creatine precursors",ids:["gly","arg","met"],why:"Creatine is not present in plant foods, so vegans synthesise it from these three amino acids. Body stores tend to run lower on a plant-based diet."},{id:"bcaa",name:"Branched-chain (BCAA)",ids:["leu","ile","val"],why:"The three amino acids muscle burns directly rather than sending to the liver. Leucine is the one that triggers muscle protein synthesis."},{id:"sulphur",name:"Sulphur amino acids",ids:["met","cys"],why:"Methionine and cysteine are scored as a pair because cysteine spares methionine. Pulses are usually short on both, which is what grains make up for."},{id:"aromatic",name:"Aromatic amino acids",ids:["phe","tyr"],why:"Phenylalanine and tyrosine are scored as a pair, since the body makes tyrosine from phenylalanine. Both feed dopamine and thyroid hormone production."},{id:"iron",name:"Iron & absorption",ids:["fe","vitc"],why:"Plant iron is non-haem and poorly absorbed on its own. Vitamin C in the same meal can multiply uptake severalfold, so the two columns are worth reading together."},{id:"bone",name:"Bone health",ids:["ca","vitd","vitk","mg","p"],why:"Calcium is only half the story: vitamin D governs how much you absorb, vitamin K directs it into bone, and magnesium and phosphorus build the mineral itself."},{id:"methyl",name:"B12, folate & methylation",ids:["b12","b9","b6","chol","met"],why:"The nutrients that keep homocysteine in check. B12 is the critical gap on a vegan diet, because unfortified plant foods are not a reliable source whatever these figures show."},{id:"omega",name:"Omega balance",ids:["ala","la"],why:"The two fats the body cannot make. They compete for the same enzymes, so a diet heavy in omega-6 blunts conversion of omega-3 into the forms the body actually uses."},{id:"antiox",name:"Antioxidant vitamins",ids:["vita","vitc","vite","se"],why:"Nutrients that limit oxidative damage, working in different compartments: vitamin C in water, vitamin E in fat, and selenium as part of the enzymes that recycle them."},{id:"electro",name:"Electrolytes",ids:["na","k","mg","ca"],why:"The minerals governing fluid balance, nerve signalling and muscle contraction. Whole plant foods are naturally high in potassium and low in sodium, the opposite of most processed food."}],DAY_MAX_G=5e3,DEFAULT_G=100,DEFAULT_KG=70,S={groups:new Set(["macro","amino"]),sort:{id:"__name",dir:1},q:"",cat:"",sel:0,favs:new Set,favsOnly:!1,dv:!1,basis:"g",view:"table",tab:"overview",chartNut:"protein",dark:!1,lens:"",custom:[],day:[],kg:DEFAULT_KG,wUnit:"kg"},clampG=e=>{const t=Math.round(Number(e));return isFinite(t)&&t>0?Math.min(t,DAY_MAX_G):0},clampKg=e=>{const t=Number(e);return!isFinite(t)||t<=0?DEFAULT_KG:Math.round(Math.min(Math.max(t,30),250)*10)/10},LB_PER_KG=2.2046226218,LB_PER_ST=14,kgToStLb=e=>{const t=Math.round(e*LB_PER_KG);return{st:Math.floor(t/LB_PER_ST),lb:t%LB_PER_ST}},stLbToKg=(e,t)=>(e*LB_PER_ST+t)/LB_PER_KG;function weightLabel(){if(S.wUnit!=="stlb")return`${+S.kg.toFixed(1)} kg`;const{st:e,lb:t}=kgToStLb(S.kg);return`${e} st ${t} lb`}const LS_KEY="vegan-nutrients:v1";let storageOK=!0;function savePrefs(){if(storageOK)try{localStorage.setItem(LS_KEY,JSON.stringify({favs:[...S.favs],groups:[...S.groups],sort:S.sort,dv:S.dv,basis:S.basis,dark:S.dark,lens:S.lens,custom:S.custom,favsOnly:S.favsOnly,cat:S.cat,chartNut:S.chartNut,day:S.day,kg:S.kg,wUnit:S.wUnit}))}catch{storageOK=!1}}const lensById=e=>e&&[...BUILTIN_LENSES,...S.custom].find(t=>t.id===e)||null,lensIds=()=>new Set(lensById(S.lens)?.ids||[]);function loadPrefs(){let e;try{e=JSON.parse(localStorage.getItem(LS_KEY)||"null")}catch{storageOK=!1;return}if(!(!e||typeof e!="object")){if(Array.isArray(e.favs)&&(S.favs=new Set(e.favs.map(currentSlug).filter(t=>BY_SLUG.has(t)))),Array.isArray(e.groups)){const t=e.groups.filter(a=>GROUPS.some(n=>n.id===a));t.length&&(S.groups=new Set(t))}e.sort&&(e.sort.id==="__name"||IDX.has(e.sort.id))&&(S.sort={id:e.sort.id,dir:e.sort.dir===1?1:-1}),typeof e.dv=="boolean"&&(S.dv=e.dv),(e.basis==="g"||e.basis==="kcal")&&(S.basis=e.basis),typeof e.dark=="boolean"&&(S.dark=e.dark),CATS.includes(e.cat)&&(S.cat=e.cat),IDX.has(e.chartNut)&&(S.chartNut=e.chartNut),Array.isArray(e.day)&&(S.day=e.day.filter(t=>t&&t.slug).map(t=>({slug:currentSlug(t.slug),g:clampG(t.g)})).filter(t=>BY_SLUG.has(t.slug))),typeof e.kg=="number"&&isFinite(e.kg)&&(S.kg=clampKg(e.kg)),(e.wUnit==="kg"||e.wUnit==="stlb")&&(S.wUnit=e.wUnit),Array.isArray(e.custom)&&(S.custom=e.custom.filter(t=>t&&typeof t.name=="string"&&Array.isArray(t.ids)).map(t=>({id:String(t.id||""),name:t.name.slice(0,40),ids:t.ids.filter(a=>IDX.has(a)),...typeof t.why=="string"&&t.why?{why:t.why.slice(0,240)}:{}})).filter(t=>t.id&&t.ids.length)),typeof e.lens=="string"&&lensById(e.lens)&&(S.lens=e.lens),S.favsOnly=e.favsOnly===!0&&S.favs.size>0}}const $=e=>document.querySelector(e),esc=e=>String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]),say=e=>{$("#live").textContent=e},cols=()=>NUTS.map((e,t)=>({...e,i:t})).filter(e=>S.groups.has(e.group)),val=(e,t)=>e.v[IDX.get(t)],GRAMS_PER=100,KCAL_BASIS=100;function gramsPer100kcal(e){const t=val(e,"kcal");return t?KCAL_BASIS/t*GRAMS_PER:null}const basisLabel=()=>S.basis==="kcal"?"per 100 kcal":"per 100 g";function shown(e,t){const a=val(e,t.id);if(S.basis==="g"||t.id==="kcal"||a===null||a===void 0)return a;const n=val(e,"kcal");return n?a/n*KCAL_BASIS:null}const NAME_COUNT=FOODS.reduce((e,t)=>e.set(t.name,(e.get(t.name)||0)+1),new Map),fullName=e=>NAME_COUNT.get(e.name)>1&&e.state?`${e.name}, ${e.state}`:e.name,isFav=e=>S.favs.has(SLUGS[e]);function toggleFav(e){isFav(e)?S.favs.delete(SLUGS[e]):S.favs.add(SLUGS[e]),S.favsOnly&&!S.favs.size&&(S.favsOnly=!1),savePrefs()}function fmtText(e,t){return e==null?"n/a":S.dv&&t.dv?e===0?"0%":Math.round(e/t.dv*100)+"%":e===0?"0":e.toFixed(t.dp)}function fmt(e,t){return e==null?'<span class="na">n/a</span>':fmtText(e,t)}const FAO_PATTERN=[{label:"Histidine",ids:["his"],mg:15},{label:"Isoleucine",ids:["ile"],mg:30},{label:"Leucine",ids:["leu"],mg:59},{label:"Lysine",ids:["lys"],mg:45},{label:"Methionine + cysteine",ids:["met","cys"],mg:22},{label:"Phenylalanine + tyrosine",ids:["phe","tyr"],mg:38},{label:"Threonine",ids:["thr"],mg:23},{label:"Tryptophan",ids:["trp"],mg:6},{label:"Valine",ids:["val"],mg:39}];function proteinQuality(e){const t=val(e,"protein");if(!t||t<1||FAO_PATTERN.some(s=>s.ids.some(i=>val(e,i)===null||val(e,i)===void 0)))return null;const a=FAO_PATTERN.map(s=>{const i=s.ids.reduce((l,o)=>l+val(e,o),0)*1e3/t;return{label:s.label,pc:i/s.mg*100}});if(a.some(s=>!isFinite(s.pc)))return null;const n=a.reduce((s,i)=>i.pc<s.pc?i:s);return{score:Math.round(n.pc),limiting:n.label,perKcal:(val(e,"kcal")??0)>0?t/val(e,"kcal")*100:null}}function omegaRatio(e){const t=val(e,"la"),a=val(e,"ala");return!t||!a?null:t>=a?{a:t/a,flip:!1}:{a:a/t,flip:!0}}const dayEntries=()=>S.day.map(e=>({...e,i:BY_SLUG.get(e.slug)})).filter(e=>e.i!==void 0).map(e=>({...e,f:FOODS[e.i]})),dayContributors=()=>dayEntries().filter(e=>e.g>0),dayGrams=()=>dayContributors().reduce((e,t)=>e+t.g,0);function dayTotals(){const e=dayContributors();return NUTS.map(t=>{let a=0,n=0;const s=new Set;for(const i of e){const l=val(i.f,t.id);if(l==null)continue;n++,a+=l*i.g/100;const o=noteFor(i.i,t.id);o&&s.add(o)}return{n:t,total:n?a:null,from:n,of:e.length,partial:n>0&&n<e.length,notes:[...s]}})}const totalOf=(e,t)=>e[IDX.get(t)],PROTEIN_G_PER_KG=.66,aaTargets=e=>FAO_PATTERN.map(t=>({label:t.label,ids:t.ids,target:t.mg*PROTEIN_G_PER_KG*e/1e3}));function dayAminoAcids(e){const t=dayContributors();return t.length?aaTargets(S.kg).map(a=>{const s=t.every(i=>a.ids.every(l=>{const o=val(i.f,l);return o!=null}))?a.ids.reduce((i,l)=>i+(totalOf(e,l).total??0),0):null;return{...a,got:s,pc:s===null?null:s/a.target*100}}):[]}function dayProteinQuality(e){const t=dayContributors();if(!t.length)return null;const a=FAO_PATTERN.flatMap(s=>s.ids);return t.every(s=>a.every(i=>{const l=val(s.f,i);return l!=null}))?proteinQuality({v:e.map(s=>s.total)}):null}const A_BUDGET=new Set(["kcal","carbs","fat","satfat","na"]);function dayStanding(e){const t=e.filter(n=>n.n.dv&&n.total!==null&&!n.partial).map(n=>({id:n.n.id,label:n.n.label,pc:n.total/n.n.dv*100,budget:A_BUDGET.has(n.n.id)})),a=(n,s)=>n.pc-s.pc;return{short:t.filter(n=>!n.budget&&n.pc<50).sort(a),over:t.filter(n=>!n.budget&&n.pc>=100).sort((n,s)=>-a(n,s)),budget:t.filter(n=>n.budget&&n.pc>=100).sort((n,s)=>-a(n,s))}}function addToDay(e,t=DEFAULT_G){if(!BY_SLUG.has(e))return;const a=S.day.find(n=>n.slug===e);a?a.g=clampG(a.g+t):S.day.push({slug:e,g:clampG(t)}),savePrefs()}function setDayGrams(e,t){const a=S.day.find(n=>n.slug===e);a&&(a.g=clampG(t),savePrefs())}function removeFromDay(e){S.day=S.day.filter(t=>t.slug!==e),savePrefs()}function rows(){const e=S.q.trim().toLowerCase();let t=FOODS.map((s,i)=>({f:s,i}));S.favsOnly&&(t=t.filter(s=>isFav(s.i))),S.cat&&(t=t.filter(s=>s.f.cat===S.cat)),e&&(t=t.filter(s=>(s.f.name+" "+(s.f.alt||"")+" "+s.f.state+" "+s.f.cat).toLowerCase().includes(e)));const{id:a,dir:n}=S.sort;return t.sort((s,i)=>{if(a==="__name")return n*s.f.name.localeCompare(i.f.name);const l=NUTS[IDX.get(a)],o=shown(s.f,l),c=shown(i.f,l);return o===c?s.f.name.localeCompare(i.f.name):o===null?1:c===null?-1:n*(o-c)}),t}function renderGroups(){const e={};NUTS.forEach(t=>e[t.group]=(e[t.group]||0)+1),$("#groupNav").innerHTML=GROUPS.map(t=>`
    ...
    <li><button class="navbtn sub" type="button" data-cat="${esc(a)}"
      ...
      <span>${esc(n)}</span>
      <span class="count">${s}</span><span class="dot"></span></button></li>`).join("")}function setCat(e){S.cat=e===S.cat?"":e,renderCats(),say(S.cat?`Showing ${S.cat} only.`:"Showing all categories."),savePrefs(),render()}function toggleGroup(e){S.groups.has(e)?S.groups.delete(e):S.groups.add(e);const t=!S.groups.size;t&&S.groups.add("macro"),document.querySelectorAll("#groupNav [data-grp]").forEach(n=>n.setAttribute("aria-pressed",String(S.groups.has(n.dataset.grp))));const a=GROUPS.find(n=>n.id===e);say(t?"The table needs at least one group, so macronutrients stay shown.":`${a.label} ${S.groups.has(e)?"shown":"hidden"}. ${cols().length} nutrient columns visible.`),savePrefs(),render()}function renderLensSelect(){const e=t=>`<option value="${esc(t.id)}"${t.id===S.lens?" selected":""}${t.why?` title="${esc(t.why)}"`:""}>${esc(t.name)}</option>`;$("#lensSel").innerHTML=`<option value=""${S.lens?"":" selected"}>None</option><optgroup label="Built in">${BUILTIN_LENSES.map(e).join("")}</optgroup>`+(S.custom.length?`<optgroup label="Yours">${S.custom.map(e).join("")}</optgroup>`:""),$("#lensSel").classList.toggle("lensactive",!!S.lens),renderLensNote()}function renderLensNote(){const e=lensById(S.lens),t=$("#lensNote");if(!e){t.hidden=!0,t.innerHTML="";return}const a=e.ids.map(n=>NUTS[IDX.get(n)]?.label).filter(Boolean);t.hidden=!1,t.innerHTML=`<b>${esc(e.name)}</b>`+(e.why?` ${esc(e.why)}`:` Highlighting ${a.length} nutrients.`)+`<span class="cols">${a.map(esc).join(" · ")}</span>`}function setLens(e){S.lens=lensById(e)?e:"";const t=lensById(S.lens);if(t){const n=[...new Set(t.ids.map(s=>NUTS[IDX.get(s)].group))].filter(s=>!S.groups.has(s));n.forEach(s=>S.groups.add(s)),n.length?(renderGroups(),say(`Highlighting ${t.name}. Also showing ${n.map(s=>GROUPS.find(i=>i.id===s).label.toLowerCase()).join(" and ")}.`)):say(`Highlighting ${t.name}, ${t.ids.length} nutrients.`)}else say("Highlight cleared.");renderLensSelect(),savePrefs(),render()}let hoverNut=null;function renderNutNote(){const e=hoverNut||(S.sort.id!=="__name"?S.sort.id:null),t=e?NUTS[IDX.get(e)]:null;$("#nutNote").innerHTML=t&&t.why?`<b>${esc(t.label)}</b> ${esc(t.why)}`:`Point at a column header, or tab onto one, to read what that nutrient
       does in the body. Sorting by a column leaves its explanation here.`}const nutOf=e=>{const t=e.target.closest?.("[data-sort]")?.dataset.sort;return t&&t!=="__name"?t:null};function previewNut(e){e!==hoverNut&&(hoverNut=e,renderNutNote())}$("#thead").addEventListener("mouseover",e=>previewNut(nutOf(e))),$("#thead").addEventListener("mouseleave",()=>previewNut(null)),$("#thead").addEventListener("focusin",e=>previewNut(nutOf(e))),$("#thead").addEventListener("focusout",()=>previewNut(null));function layout(){const e=cols(),t=lensIds();return e.map((a,n)=>{const s=t.has(a.id);return{...a,gstart:n===0||e[n-1].group!==a.group,lens:s,lensL:s&&(n===0||!t.has(e[n-1].id)),lensR:s&&(n===e.length-1||!t.has(e[n+1].id)),sorted:S.sort.id===a.id}})}const colClass=e=>[e.gstart&&"gstart",e.lens&&"lens",e.lensL&&"lensL",e.lensR&&"lensR",e.sorted&&"sorted"].filter(Boolean).join(" ");function renderTable(e){const t=layout(),a=e,n=S.sort.id==="__name",s=lensIds(),i=GROUPS.filter(r=>S.groups.has(r.id)).map(r=>{const d=t.filter(m=>m.group===r.id);return`<th class="grp${d.some(m=>m.lens)?" lens":""}" data-g="${r.id}" colspan="${d.length}"
      scope="colgroup"><span class="grplabel">${esc(r.label)}</span></th>`}).join(""),l=t.map(r=>{const d=r.sorted?S.sort.dir===1?"ascending":"descending":"none",p=S.dv&&r.dv?"%DV":r.unit;return`<th scope="col" aria-sort="${d}" data-g="${r.group}" class="${colClass(r)}">
    ...
    <tr>
      ...
      ${i}
    </tr><tr>${l}</tr>`;const o=new Set;$("#tbody").innerHTML=a.length?a.map(({f:r,i:d})=>`
    <tr data-i="${d}" ${S.sel===d?'aria-selected="true"':""}>
      <td class="food${n?" sorted":""}"><div class="fcell">
        <span class="sw" style="--c:${r.colour}" aria-hidden="true"></span>
        <button class="fname" type="button" data-pick="${d}" data-name="${esc(r.name)}">
          <b>${esc(r.name)}${r.alt?` <span class="alt">(${esc(r.alt)})</span>`:""}</b>
          ${r.state?`<span>${esc(r.state)}</span>`:""}
          ${S.basis==="kcal"&&gramsPer100kcal(r)!==null?`<span class="per100">${Math.round(gramsPer100kcal(r))} g</span>
               <span class="sr">makes 100 kcal</span>`:""}
          <span class="sr">, show full profile</span></button>
        <button class="fav" type="button" data-fav="${d}" aria-pressed="${isFav(d)}">
          ${isFav(d)?I.heartFull:I.heart}
          <span class="sr">${isFav(d)?"Remove":"Add"} ${esc(r.name)} ${isFav(d)?"from":"to"} favourites</span>
        </button></div></td>
      ${t.map(p=>{const m=shown(r,p),f=m===0||m===null,u=m===null?null:noteFor(d,p.id);return u&&o.add(u),`<td class="num${f?" low":""} ${colClass(p)}" data-g="${p.group}">${fmt(m,p)}${u?noteMark(u):""}</td>`}).join("")}
    </tr>`).join(""):`<tr><td class="empty" colspan="${t.length+1}">${emptyState()}</td></tr>`;const c=lensById(S.lens);$("#cap").textContent=`${FOODS.length} vegan foods, ${t.length} nutrient columns. Values ${basisLabel()} of food`+(S.dv?", shown as % of adult daily value.":".")+(c?` ${c.name} highlighted.`:"");const h=$("#noteKey");h.hidden=!o.size,h.innerHTML=[...o].map(r=>`<span><sup class="fnote">${esc(r.marker)}</sup> <b>${esc(r.short)}.</b>
     ${esc(r.text)}</span>`).join(""),syncHeadOffset()}function syncHeadOffset(){const e=$("#thead").rows[0];if(!e)return;const t=e.getBoundingClientRect().height;t&&$("#grid").style.setProperty("--head1",`${Math.floor(t)}px`);const a=e.cells[0],n=a&&a.getBoundingClientRect().width;n&&$("#grid").style.setProperty("--foodw",`${n}px`)}addEventListener("resize",syncHeadOffset);function emptyState(){return S.favsOnly&&!S.favs.size?`<b>No favourites yet</b>Star a food with the heart button to build a shortlist,
          ...
          <div style="margin-top:14px"><button class="btn" type="button" data-act="clearfilters">
            Clear search and category</button></div>`}function renderChart(e){const t=NUTS[IDX.get(S.chartNut)],a=e.slice().sort((s,i)=>(val(i.f,t.id)??-1)-(val(s.f,t.id)??-1)),n=Math.max(...a.map(s=>val(s.f,t.id)??0),1e-4);$("#chartNut").innerHTML=GROUPS.filter(s=>S.groups.has(s.id)).map(s=>`<optgroup label="${s.label}">`+NUTS.filter(i=>i.group===s.id).map(i=>`<option value="${i.id}"${i.id===S.chartNut?" selected":""}>${esc(i.label)}</option>`).join("")+"</optgroup>").join(""),$("#chartRows").innerHTML=a.slice(0,25).map(({f:s})=>{const i=val(s,t.id)??0;return`<div class="crow">
      <span class="lbl"><span class="sw" style="--c:${s.colour}" aria-hidden="true"></span>
        <span title="${esc(fullName(s))}">${esc(fullName(s))}</span></span>
      <span class="track"><i style="width:${(i/n*100).toFixed(1)}%"></i></span>
      <span class="val">${fmt(i,t)} <span class="unit">${S.dv&&t.dv?"":t.unit}</span></span>
    </div>`}).join(""),$("#chartRows").setAttribute("role","img"),$("#chartRows").setAttribute("aria-label",`Bar chart of ${t.label} across ${Math.min(a.length,25)} foods, highest first. `+a.slice(0,25).map(({f:s})=>`${fullName(s)} ${fmtText(val(s,t.id),t)}`).join(", "))}function proteinQualityBlock(e){const t=proteinQuality(e),a=omegaRatio(e),n=NUTS.some(l=>l.group==="amino"&&val(e,l.id)===null);if(!t&&n&&(val(e,"protein")??0)>=1)return`<h4 style="margin-top:18px">Protein quality</h4>
      ...
      in the source data, not in the food.</p>`;if(!t&&!a)return"";let s="";if(t){const l=t.score>=100;s+=`<div class="drow"><dt>Amino acid score</dt>
        <dd class="${l?"pc":""}">${t.score}%</dd></div>
      <div class="drow"><dt>${l?"Lowest relative to need":"Limiting amino acid"}</dt>
      ...
      <dd>${a.flip?`1 : ${a.a.toFixed(1)}`:`${a.a.toFixed(1)} : 1`}</dd></div>`);const i=t?t.score>=100?"Meets the adult FAO/WHO pattern for every essential amino acid.":`Scored against the FAO/WHO adult pattern. ${esc(t.limiting)} caps the
           score; pairing this food with one richer in it raises the total.`:"";return`<h4 style="margin-top:18px">Protein quality</h4><dl>${s}</dl>`+(i?`<p style="font-size:11.5px;color:var(--faint);margin:8px 0 0;line-height:1.4">${i}</p>`:"")}function renderDetail(){const e=FOODS[S.sel],t=o=>shown(e,NUTS[IDX.get(o)]),a=S.day.find(o=>o.slug===SLUGS[S.sel]),n=["vitamin","mineral","amino","plant"],s=[["overview","Overview",I.macro],...n.map(o=>GROUPS.find(c=>c.id===o)).filter(Boolean).map(o=>[o.id,o.label,o.icon])],i=new Set;let l;if(S.tab==="overview"){const o=["kcal","protein","carbs","fiber","fat","satfat"],c=NUTS.filter(h=>h.dv&&h.group!=="macro").map(h=>({n:h,pc:(t(h.id)??0)/h.dv*100})).filter(h=>h.pc>0).sort((h,r)=>r.pc-h.pc).slice(0,6);l="<h4>Macronutrients</h4><dl>"+o.map(h=>{const r=NUTS[IDX.get(h)],d=h==="fiber"||h==="satfat",p=h==="kcal"&&t("kcal")!==null?` <span class="pc">· ${Math.round(t("kcal")*4.184)} kJ</span>`:"";return`<div class="drow${d?" sub":""}"><dt>${esc(r.label)}</dt>
        <dd>${(t(h)??0).toFixed(r.dp)} ${r.unit}${p}</dd></div>`}).join("")+"</dl>"+proteinQualityBlock(e)+'<h4 style="margin-top:18px">Top nutrients</h4><dl>'+c.map(({n:h,pc:r})=>{const d=noteFor(S.sel,h.id);return d&&i.add(d),`<div class="drow"><dt>${esc(h.label)}</dt>
          <dd class="pc">${Math.round(r)}% DV${d?noteMark(d):""}</dd></div>`}).join("")+"</dl>"}else{const o=NUTS.filter(d=>d.group===S.tab),c=Math.max(...o.map(d=>t(d.id)??0),1e-4),h=lensIds(),r=o.filter(d=>t(d.id)===null||t(d.id)===void 0).length;l=`<h4>${esc(GROUPS.find(d=>d.id===S.tab)?.label||S.tab)}</h4>
      <dl>`+o.map(d=>{const p=t(d.id),m=p==null,f=!m&&d.dv?Math.round(p/d.dv*100):null,u=m?null:noteFor(S.sel,d.id);return u&&i.add(u),`<div class="drow${h.has(d.id)?" lensrow":""}" style="display:block">
          <div style="display:flex;justify-content:space-between;gap:10px">
            <dt>${esc(d.label)}</dt>
            <dd>${m?'<span class="nodata">not measured</span>':`${p.toFixed(d.dp)} ${d.unit}${f!==null?` <span class="pc">· ${f}%</span>`:""}${u?noteMark(u):""}`}</dd>
          </div>
          ${m?"":`<div class="minibar" aria-hidden="true"><i style="width:${(p/c*100).toFixed(1)}%"></i></div>`}
        </div>`}).join("")+"</dl>"+(r?`<p class="nodatanote">${r===o.length?"USDA publishes no figures at all for this group in this food.":`USDA publishes no figure for ${r} of the ${o.length}.`}
    ...
    <div class="tabs" role="tablist" aria-label="Nutrient detail sections">
      ${s.map(([o,c,h])=>`
        <button type="button" role="tab" data-tab="${o}" id="tab-${o}"
          aria-selected="${S.tab===o}" aria-controls="tabp"
          tabindex="${S.tab===o?0:-1}">${h}<span>${c}</span></button>`).join("")}
    </div>
    <div class="dbody" id="tabp" role="tabpanel" aria-labelledby="tab-${S.tab}" tabindex="0">${l}${[...i].map(o=>`<p class="nodatanote"><sup class="fnote">${esc(o.marker)}</sup>
        <b>${esc(o.short)}.</b> ${esc(o.text)}</p>`).join("")}</div>
    <div class="dfoot">% DV uses general adult reference values. Yours may differ.</div>`}function renderMeta(e){const t=cols().length,a=lensById(S.lens);$("#meta").innerHTML=`${I.info} Showing <b>${e}</b> of ${FOODS.length} foods ·
    <b>${t}</b> of ${NUTS.length} nutrients`+(S.favsOnly?" · favourites only":"")+(S.dv?" · % daily value":"")+(S.basis==="kcal"?" · per 100 kcal":"")+(S.dv&&S.basis==="kcal"?` · <span class="lenshint">5% here is a full day's worth</span> at 2000 kcal`:"")+(a?` · <span class="lenshint">${esc(a.name)}</span> highlighted`:""),$("#favCount").textContent=S.favs.size||"",$("#dayCount").textContent=dayEntries().length||""}const fmtTotal=(e,t)=>e===null?"not measured":`${e.toFixed(t.dp)} ${t.unit}`;function renderDayList(){const e=dayEntries(),t=$("#dayList");if(!e.length){t.innerHTML=`<div class="dayempty">
    ...
    <div class="dayfoot">
      ...
      <span class="push">${e.length} food${e.length===1?"":"s"} ·
    ...
    </div>`}const FAO_SCORED=new Set(FAO_PATTERN.flatMap(e=>e.ids)),NON_ESSENTIAL=NUTS.filter(e=>e.group==="amino"&&!FAO_SCORED.has(e.id)).length;function aminoRefsByNutrient(e){const t=new Map;for(const a of dayAminoAcids(e))for(const n of a.ids)t.set(n,{...a,partners:a.ids.filter(s=>s!==n)});return t}function renderDayTotals(e){const t=dayContributors(),a=$("#dayTotals");if(!t.length){a.innerHTML="";return}const n=aminoRefsByNutrient(e),s=new Set,i=GROUPS.filter(l=>S.groups.has(l.id)).map(l=>{const o=e.filter(h=>h.n.group===l.id).map(h=>{const{n:r,total:d,partial:p,from:m,of:f,notes:u}=h,g=n.get(r.id),y=r.dv&&d!==null?d/r.dv*100:g?g.pc:null,v=g&&g.partners.length?`<span class="qual">with ${g.partners.map(b=>NUTS[IDX.get(b)].label.toLowerCase()).join(" and ")}</span>`:"";return u.forEach(b=>s.add(b)),`<div class="totrow${d===null?" none":""}">
        <span class="totname">${esc(r.label)}${u.map(noteMark).join("")}${v}</span>
        <span class="totval">${d===null?'<span class="nodata">not measured</span>':`${d.toFixed(r.dp)} <span class="u">${esc(r.unit)}</span>`}</span>
        ...
        <span class="totpc">${y===null?`<span class="noref" aria-hidden="true">&ndash;</span>
             <span class="sr">no ${r.group==="amino"?"published requirement":"daily value published"}</span>`:`${Math.round(y)}%`}</span>
        <span class="totcov">${p?`from ${m} of ${f}`:""}</span>
      </div>`}).join(""),c=l.id==="amino";return`<div class="totgroup" data-g="${l.id}"><h4>${l.icon}${esc(l.label)}</h4>
      <div class="tothead" aria-hidden="true">
        <span>Nutrient</span><span>Total</span><span></span>
        <span>${c?"of requirement":"of daily value"}</span><span></span>
      </div>${o}
      ${c?`<p class="nodatanote">Against the FAO/WHO adult requirement for
    ...
    </div>`}).join("");a.innerHTML=`<div class="totals">${i}</div>`+(s.size?`<div class="notekey">${[...s].map(l=>`<span><sup class="fnote">${esc(l.marker)}</sup> <b>${esc(l.short)}.</b>
       ${esc(l.text)}</span>`).join("")}</div>`:"")+`<p class="nodatanote" style="margin-top:12px">Percentages use the same general adult
    ...
    <dd>${t.got===null?'<span class="nodata">not measured</span>':`${t.got.toFixed(2)} g <span class="pc">· ${Math.round(t.pc)}%</span>`}</dd></div>`).join("");function weightRow(){const e=(i,l)=>`<button type="button" data-wunit="${i}" aria-pressed="${S.wUnit===i}">${l}</button>`,t=(i,l,o,c,h)=>`<input type="number" inputmode="numeric" id="${i}" data-w value="${l}"
       min="0"${c?` max="${c}"`:""} step="1" aria-label="Body weight in ${h}">
     <span class="u">${o}</span>`,{st:a,lb:n}=kgToStLb(S.kg),s=S.wUnit==="stlb"?t("dayStones",a,"st",40,"stones")+t("dayPounds",n,"lb",null,"pounds"):t("dayKg",+S.kg.toFixed(1),"kg",250,"kilograms");return`<div class="kgrow">
  ...
  </div>`}function readWeight(){return S.wUnit!=="stlb"?Number($("#dayKg").value):stLbToKg(Number($("#dayStones").value)||0,Number($("#dayPounds").value)||0)}function renderDaySummary(e){const t=dayContributors(),a=$("#daySum"),n='<div class="dayadvice">'+DAY_NOTES.map(([u,g])=>`<div><b>${esc(u)}</b> ${esc(g.replace(/\s+/g," "))}</div>`).join("")+"</div>";if(!t.length){a.innerHTML=`<div class="dhead"><h3>Your day</h3>
      <div class="per">nothing added yet</div></div>
      <div class="dbody"><p class="nodatanote" style="margin-top:0">Add a food and its
      totals appear here, in units and as a percentage of a daily value.</p>${n}</div>`;return}const s=totalOf(e,"kcal"),i=totalOf(e,"protein"),l=totalOf(e,"fiber"),o=[s,i,l].map(u=>{const g=u.n.dv&&u.total!==null?Math.round(u.total/u.n.dv*100):null;return`<div class="drow"><dt>${esc(u.n.label)}</dt>
      <dd>${fmtTotal(u.total,u.n)}${g===null?"":` <span class="pc">· ${g}%</span>`}</dd></div>`}).join(""),c=dayProteinQuality(e),r=["ala","la"].every(u=>!totalOf(e,u).partial)?omegaRatio({v:e.map(u=>u.total)}):null,{short:d,over:p,budget:m}=dayStanding(e),f=u=>`<button class="jump" type="button" data-daysort="${esc(u.id)}">
    <span>${esc(u.label)}</span><b>${Math.round(u.pc)}%</b>
    ...
    <div class="dbody">
      ...
      ${r?`<h4 style="margin-top:18px">Omega balance</h4><dl>
        <div class="drow"><dt>Omega-6 : omega-3</dt>
          <dd>${r.flip?`1 : ${r.a.toFixed(1)}`:`${r.a.toFixed(1)} : 1`}</dd></div></dl>`:""}

      ${d.length?`<h4 style="margin-top:18px">Short on</h4>
        <div class="jumps">${d.slice(0,8).map(f).join("")}</div>
      ...
      ${p.length?`<h4 style="margin-top:18px">Comfortable</h4>
        <div class="jumps">${p.slice(0,8).map(f).join("")}</div>`:""}

      ${m.length?`<h4 style="margin-top:18px">Above the guideline</h4>
        <div class="jumps">${m.map(f).join("")}</div>
    ...
    <div class="dfoot">Totals cover only what you have listed. A gap here is as likely to mean
      a food you have not added as a nutrient you are short of.</div>`}function renderDay(){const e=dayTotals();renderDayList(),renderDayTotals(e),renderDaySummary(e)}function render(){const e=S.view==="chart",t=S.view==="day";$("#browseView").hidden=t,$("#dayView").hidden=!t,$("#tableView").hidden=e||t,$("#chartView").hidden=!e,$("#vTable").setAttribute("aria-pressed",String(S.view==="table")),$("#vChart").setAttribute("aria-pressed",String(e)),document.querySelector('[data-act="favs"]').setAttribute("aria-pressed",String(S.favsOnly)),$("#vDay").setAttribute("aria-pressed",String(t)),t?$("#navFoods").removeAttribute("aria-current"):$("#navFoods").setAttribute("aria-current","true");for(const n of["#viewGrp",".lensgrp","#dvBtn","#nutNote","#meta"])$(n).hidden=t;t?$("#lensNote").hidden=!0:renderLensNote();const a=rows();a.length&&!a.some(n=>n.i===S.sel)&&(S.sel=a[0].i),renderTable(a),e&&renderChart(a),renderDetail(),renderDay(),renderMeta(a.length),renderNutNote()}document.addEventListener("click",e=>{const t=e.target.closest("button");if(t){if(t.dataset.grp)return toggleGroup(t.dataset.grp);if(t.dataset.cat!==void 0)return setCat(t.dataset.cat);if(t.dataset.sort){const a=t.dataset.sort;S.sort.id===a?S.sort.dir*=-1:S.sort={id:a,dir:a==="__name"?1:-1};const n=a==="__name"?"Food name":NUTS[IDX.get(a)].label;return say(`Sorted by ${n}, ${S.sort.dir===1?"ascending":"descending"}.`),savePrefs(),render()}if(t.dataset.pick!==void 0)return S.sel=+t.dataset.pick,say(`${FOODS[S.sel].name} selected.`),render();if(t.dataset.fav!==void 0){const a=+t.dataset.fav;return toggleFav(a),say(`${FOODS[a].name} ${isFav(a)?"added to":"removed from"} favourites.`),render()}if(t.dataset.tab)return S.tab=t.dataset.tab,renderDetail(),document.querySelector(`[data-tab="${S.tab}"]`)?.focus();if(t.dataset.act==="favs")return S.favsOnly=!S.favsOnly,say(S.favsOnly?`Showing favourites only, ${S.favs.size} food${S.favs.size===1?"":"s"}.`:"Showing all foods."),savePrefs(),render();if(t.dataset.act==="clearfilters")return S.q="",$("#q").value="",$("#qClear").hidden=!0,S.cat="",renderCats(),savePrefs(),say("Search and category cleared."),render();if(t.dataset.dlg)return openDialog(t.dataset.dlg);if(t.dataset.dayadd!==void 0){const a=+t.dataset.dayadd,n=!!t.closest("#daySug");addToDay(SLUGS[a],DEFAULT_G);const s=S.day.find(i=>i.slug===SLUGS[a]);return say(`${FOODS[a].name} in your day at ${s.g} g.`),n&&($("#dayQ").value="",$("#daySug").hidden=!0),render(),n&&document.querySelector(`[data-dayg="${SLUGS[a]}"]`)?.focus()}if(t.dataset.dayrm){const a=FOODS[BY_SLUG.get(t.dataset.dayrm)];return removeFromDay(t.dataset.dayrm),say(`${a?a.name:"Food"} removed from your day.`),render()}if(t.dataset.daystep){const a=t.dataset.daystep,n=S.day.find(s=>s.slug===a);return n?(setDayGrams(a,n.g+ +t.dataset.by),render()):void 0}if(t.dataset.wunit)return t.dataset.wunit===S.wUnit?void 0:(S.wUnit=t.dataset.wunit==="stlb"?"stlb":"kg",savePrefs(),renderDaySummary(dayTotals()),say(`Body weight in ${S.wUnit==="stlb"?"stones and pounds":"kilograms"}, ${weightLabel()}.`),document.querySelector("[data-w]")?.focus());if(t.dataset.act==="dayclear"){const a=S.day.length;return S.day=[],savePrefs(),say(`Cleared ${a} food${a===1?"":"s"} from your day.`),render()}if(t.dataset.daysort){const a=t.dataset.daysort,n=NUTS[IDX.get(a)];S.view="table",S.sort={id:a,dir:-1};const s=!S.groups.has(n.group);return s&&(S.groups.add(n.group),renderGroups()),savePrefs(),render(),$("#scroller").scrollIntoView({block:"nearest"}),say(`Showing the table sorted by ${n.label}, highest first.`+(s?` Also showing ${GROUPS.find(i=>i.id===n.group).label.toLowerCase()}.`:""))}}}),$("#vTable").onclick=()=>{S.view="table",render()},$("#vChart").onclick=()=>{S.view="chart",render()},$("#vDay").onclick=()=>{S.view=S.view==="day"?"table":"day",say(S.view==="day"?"Showing your day.":"Showing the food table."),render()},$("#navFoods").addEventListener("click",()=>{S.view==="day"&&(S.view="table",render())}),$("#chartNut").onchange=e=>{S.chartNut=e.target.value,savePrefs(),renderChart(rows())},$("#lensSel").onchange=e=>setLens(e.target.value),$("#basisBtn").onclick=e=>{S.basis=S.basis==="g"?"kcal":"g";const t=S.basis==="kcal";e.currentTarget.setAttribute("aria-pressed",String(t)),e.currentTarget.lastChild.textContent=t?" Show per 100 g":" Show per 100 kcal",say(t?"Showing figures per 100 kcal.":"Showing figures per 100 g."),savePrefs(),render()},$("#dvBtn").onclick=e=>{S.dv=!S.dv,e.currentTarget.setAttribute("aria-pressed",String(S.dv)),e.currentTarget.lastChild.textContent=S.dv?" Show raw amounts":" Show % daily value",say(S.dv?"Showing percentage of daily value.":"Showing raw amounts."),savePrefs(),render()},$("#resetBtn").onclick=()=>{S.groups=new Set(["macro","amino"]),S.sort={id:"protein",dir:-1},S.q="",$("#q").value="",$("#qClear").hidden=!0,S.cat="",S.dv=!1,$("#dvBtn").setAttribute("aria-pressed","false"),$("#dvBtn").lastChild.textContent=" Show % daily value",S.basis="g",$("#basisBtn").setAttribute("aria-pressed","false"),$("#basisBtn").lastChild.textContent=" Show per 100 kcal",S.favsOnly=!1,S.lens="",savePrefs(),renderGroups(),renderCats(),renderLensSelect(),render(),say("Columns and filters reset. Favourites, your day and saved highlight groups kept.")};let qt;$("#q").oninput=e=>{S.q=e.target.value,$("#qClear").hidden=!S.q,clearTimeout(qt),qt=setTimeout(()=>{render(),say(`${rows().length} foods match.`)},160)},$("#qClear").onclick=()=>{S.q="",$("#q").value="",$("#qClear").hidden=!0,$("#q").focus(),render()};function applyTheme(){document.documentElement.dataset.theme=S.dark?"dark":"",$("#themeBtn").setAttribute("aria-pressed",String(S.dark)),$("#themeIc").innerHTML=S.dark?I.sun:I.moon,$("#themeTx").textContent=S.dark?"Light mode":"Dark mode"}$("#themeBtn").onclick=()=>{S.dark=!S.dark,applyTheme(),savePrefs()};const DAY_SUGGESTIONS=8;function daySuggestions(e){const t=e.trim().toLowerCase();if(!t)return[];const a=FOODS.map((s,i)=>({f:s,i})).filter(({f:s})=>`${s.name} ${s.alt||""} ${s.state||""} ${s.cat}`.toLowerCase().includes(t)),n=({f:s,i})=>(isFav(i)?0:2)+(s.name.toLowerCase().startsWith(t)?0:1);return a.sort((s,i)=>n(s)-n(i)||s.f.name.localeCompare(i.f.name)).slice(0,DAY_SUGGESTIONS)}function renderDaySuggestions(){const e=$("#daySug"),t=daySuggestions($("#dayQ").value);e.hidden=!t.length,$("#dayQ").setAttribute("aria-expanded",String(!!t.length)),e.innerHTML=t.map(({f:a,i:n})=>{const s=S.day.some(i=>i.slug===SLUGS[n]);return`<button type="button" role="option" aria-selected="false" data-dayadd="${n}">
      <span class="sw" style="--c:${a.colour}" aria-hidden="true"></span>
      ...
      <span class="sr">${s?`already in your day, add another ${DEFAULT_G} grams`:`add ${DEFAULT_G} grams`}</span></button>`}).join("")}$("#dayQ").oninput=renderDaySuggestions,$("#dayQ").onfocus=renderDaySuggestions,$("#dayQ").onkeydown=e=>{if(e.key==="Escape"){$("#daySug").hidden=!0,$("#dayQ").setAttribute("aria-expanded","false");return}if(e.key==="ArrowDown")return e.preventDefault(),$("#daySug button")?.focus();e.key==="Enter"&&(e.preventDefault(),$("#daySug button")?.click())},$("#daySug").addEventListener("keydown",e=>{const t=[...$("#daySug").querySelectorAll("button")],a=t.indexOf(e.target);a!==-1&&(e.key==="ArrowDown"&&(e.preventDefault(),t[(a+1)%t.length].focus()),e.key==="ArrowUp"&&(e.preventDefault(),(a?t[a-1]:$("#dayQ")).focus()),e.key==="Escape"&&(e.preventDefault(),$("#daySug").hidden=!0,$("#dayQ").focus()))}),document.addEventListener("focusin",e=>{!$("#dayView").hidden&&!e.target.closest(".dayadd")&&($("#daySug").hidden=!0,$("#dayQ").setAttribute("aria-expanded","false"))}),$("#dayList").addEventListener("input",e=>{const t=e.target.dataset?.dayg;if(!t)return;setDayGrams(t,e.target.value);const a=dayTotals();renderDayTotals(a),renderDaySummary(a)}),$("#dayList").addEventListener("change",e=>{e.target.dataset?.dayg&&render()}),$("#daySum").addEventListener("input",e=>{if(e.target.dataset?.w===void 0)return;S.kg=clampKg(readWeight()),savePrefs();const t=dayTotals();$("#aaRows").innerHTML=aminoRows(t),$("#aaKg").textContent=`against FAO/WHO for ${weightLabel()}`,renderDayTotals(t)}),$("#daySum").addEventListener("change",e=>{if(e.target.dataset?.w!==void 0)if(S.wUnit==="stlb"){const{st:t,lb:a}=kgToStLb(S.kg);$("#dayStones").value=t,$("#dayPounds").value=a}else $("#dayKg").value=+S.kg.toFixed(1)}),document.addEventListener("keydown",e=>{if(!e.target.matches?.('[role="tab"]'))return;const t=[...document.querySelectorAll('[role="tab"]')],a=t.indexOf(e.target);let n=null;e.key==="ArrowRight"&&(n=(a+1)%t.length),e.key==="ArrowLeft"&&(n=(a-1+t.length)%t.length),e.key==="Home"&&(n=0),e.key==="End"&&(n=t.length-1),n!==null&&(e.preventDefault(),S.tab=t[n].dataset.tab,renderDetail(),document.querySelector(`[data-tab="${S.tab}"]`).focus())});const csvQuote=e=>`"${String(e).replace(/"/g,'""')}"`;function download(e,t){const a=new Blob(["\uFEFF"+e.join(`\r
`)],{type:"text/csv;charset=utf-8"}),n=Object.assign(document.createElement("a"),{href:URL.createObjectURL(a),download:t});n.click(),URL.revokeObjectURL(n.href)}function csvTable(){const e=cols(),t=rows(),a=csvQuote,n=basisLabel(),i=[["Food","Also known as","State","Category",...e.map(l=>`${l.label} (${S.dv&&l.dv?"%DV":l.unit} ${n})`)].map(a).join(",")].concat(t.map(({f:l})=>[a(l.name),a(l.alt||""),a(l.state),a(l.cat),...e.map(o=>{const c=shown(l,o);return c===null?"":S.dv&&o.dv?Math.round(c/o.dv*100):c})].join(",")));download(i,"vegan-nutrients.csv"),say(`Exported ${t.length} foods and ${e.length} nutrients as CSV.`)}function csvDay(){const e=cols(),t=dayTotals(),a=dayContributors(),n=csvQuote,s=o=>t[IDX.get(o)],l=[["Food","State","Grams",...e.map(o=>`${o.label} (${o.unit})`)].map(n).join(",")];for(const{f:o,g:c}of a)l.push([n(o.name),n(o.state||""),c,...e.map(h=>{const r=val(o,h.id);return r===null?"":+(r*c/100).toFixed(6)})].join(","));l.push([n("Total"),n(""),dayGrams(),...e.map(o=>s(o.id).total===null?"":+s(o.id).total.toFixed(6))].join(",")),l.push([n("% of daily value"),n(""),"",...e.map(o=>o.dv&&s(o.id).total!==null?Math.round(s(o.id).total/o.dv*100):"")].join(",")),l.push([n("Foods measured"),n(""),"",...e.map(o=>`${s(o.id).from} of ${s(o.id).of}`).map(n)].join(",")),download(l,"my-day.csv"),say(`Exported your day, ${a.length} foods and ${e.length} nutrients, as CSV.`)}const csv=()=>S.view==="day"?csvDay():csvTable();$("#csvBtn").onclick=csv;function renderNutPick(e=new Set){$("#nutPick").innerHTML=GROUPS.map(t=>{const a=NUTS.filter(n=>n.group===t.id);return`<div class="lensgroup"><h5>${esc(t.label)}</h5><div class="nutgrid">`+a.map(n=>`<label><input type="checkbox" name="nut" value="${esc(n.id)}"
      ...
      <button class="rm" type="button" data-rmlens="${esc(t.id)}">${I.x}
        <span class="sr">Delete ${esc(t.name)}</span></button></div>`).join("")}function openLensEditor(){$("#lensErr").textContent="",$("#lensName").value="",$("#lensWhy").value="",renderSavedLenses(),renderNutPick(lensIds()),$("#lensDlg").showModal(),$("#lensName").focus()}$("#lensEdit").onclick=openLensEditor,$("#lensCancel").onclick=()=>$("#lensDlg").close(),$("#lensX").onclick=()=>$("#lensDlg").close(),$("#nutPick").addEventListener("change",updateLensCount),$("#savedLenses").addEventListener("click",e=>{const t=e.target.closest("[data-rmlens]");if(!t)return;const a=t.dataset.rmlens,n=S.custom.find(s=>s.id===a);S.custom=S.custom.filter(s=>s.id!==a),S.lens===a&&(S.lens=""),savePrefs(),renderSavedLenses(),renderLensSelect(),render(),say(`Deleted highlight group ${n?n.name:""}.`)}),$("#lensForm").addEventListener("submit",e=>{e.preventDefault();const t=$("#lensName").value.trim(),a=pickedNuts();if(!t){$("#lensErr").textContent="Give the group a name.",$("#lensName").focus();return}if(!a.length){$("#lensErr").textContent="Pick at least one nutrient.";return}const n="c"+Date.now().toString(36),s=$("#lensWhy").value.trim().slice(0,240);S.custom.push({id:n,name:t.slice(0,40),ids:a,...s?{why:s}:{}}),savePrefs(),$("#lensDlg").close(),setLens(n),say(`Saved highlight group ${t}, ${a.length} nutrients.`)});const AMINO_IDS=NUTS.filter(e=>e.group==="amino").map(e=>e.id),aminoGaps=e=>AMINO_IDS.filter(t=>val(e,t)===null).length,NO_AMINOS=FOODS.filter(e=>aminoGaps(e)===AMINO_IDS.length),PART_AMINOS=FOODS.filter(e=>aminoGaps(e)>0&&aminoGaps(e)<AMINO_IDS.length),andList=e=>e.slice().sort().join(", ").replace(/, ([^,]*)$/," and $1"),FORTIFIED=NOTES.find(e=>e.id==="fortified"),FORTIFIED_FOODS=Object.keys(FORTIFIED?.cells||{}).map(e=>FOODS[BY_SLUG.get(e)]).filter(Boolean),FLAV_IDS=["anthocyanidins","flavan3ols","flavonols"],FLAV_REACHED=FOODS.filter(e=>FLAV_IDS.some(t=>val(e,t)!==null)).length,UNDIFF=NOTES.find(e=>e.id==="undifferentiated"),UNDIFF_CELLS=Object.values(UNDIFF?.cells||{}).flat().length,DLG={how:["How to use",`
    ...
    supplement or reliably fortified food is standard advice on a vegan diet.</p>`]};let lastFocus=null;function openDialog(e){const[t,a]=DLG[e];lastFocus=document.activeElement,$("#dlgT").textContent=t,$("#dlgB").innerHTML=a,$("#dlg").showModal()}$("#dlgX").onclick=()=>$("#dlg").close(),$("#dlg").addEventListener("close",()=>lastFocus?.focus()),$("#dlg").addEventListener("click",e=>{e.target.id==="dlg"&&$("#dlg").close()}),$("#totalFoods").textContent=FOODS.length;const GROUP_BLURB={macro:"macronutrients",fats:"fat fractions",amino:"amino acids",vitamin:"vitamins",mineral:"minerals",plant:"plant compounds"};$("#compBlurb").textContent=`${NUTS.length} nutrients per food: `+GROUPS.map(e=>`${NUTS.filter(t=>t.group===e.id).length} ${GROUP_BLURB[e.id]}`).join(", ").replace(/, ([^,]*)$/," and $1")+".",S.dark=matchMedia("(prefers-color-scheme: dark)").matches,loadPrefs(),applyTheme(),$("#dvBtn").setAttribute("aria-pressed",String(S.dv)),S.dv&&($("#dvBtn").lastChild.textContent=" Show raw amounts"),$("#basisBtn").setAttribute("aria-pressed",String(S.basis==="kcal")),S.basis==="kcal"&&($("#basisBtn").lastChild.textContent=" Show per 100 g"),renderGroups(),renderCats(),renderLensSelect(),render(),S.favs.size&&say(`${S.favs.size} saved favourite${S.favs.size===1?"":"s"} restored.`);
